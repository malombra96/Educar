...
public class M10A38BehaviourGroupInputField : MonoBehaviour
{
        ...
        for (int i = 0; i < _inputFields.Count; i++)
        {
            for (int j = 0; j < temporary.Count; j++)
            {
                if (temporary.Contains(_inputFields[i].text))
                {
                    ...
                    {
                        _inputFields[i].GetComponent<M10A38BehaviourInputField>()._isRight = true;
                ...
                {
                    _inputFields[i].GetComponent<M10A38BehaviourInputField>()._isRight = false;
                    break;
                }
