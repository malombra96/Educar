...
public class M10A38ManagerInputField : MonoBehaviour
{    
    ...
    [HideInInspector] public List<M10A38BehaviourGroupInputField> _groupInputField;
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M10A38BehaviourInputField>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M10A38BehaviourInputField>()._isRight = false;
                inputField.GetComponent<M10A38BehaviourInputField>()._isEnabled = true;
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == M10A38BehaviourGroupInputField.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<M10A38BehaviourInputField>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<M10A38BehaviourInputField>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<M10A38BehaviourInputField>()._isRight;
