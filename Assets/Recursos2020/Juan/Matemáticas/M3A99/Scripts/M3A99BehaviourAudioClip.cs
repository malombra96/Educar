...
public class M3A99BehaviourAudioClip : MonoBehaviour
{
    ...
    private void Update()
    {
        if (GetComponent<AudioSource>().time >= (duracionAudio - 0.1f)|| !toggleAudio.isOn) // Cuanto termina el audio del layout
        {
            PausaLayaout(false);
        }
    }
