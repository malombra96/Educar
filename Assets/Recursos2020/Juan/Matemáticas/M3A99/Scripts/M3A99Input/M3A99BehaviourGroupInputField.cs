...
public class M3A99BehaviourGroupInputField : MonoBehaviour
{
    M3A99ManagerInputField _managerInputField;
    ...
    void Awake()
    {
        _managerInputField = FindObjectOfType<M3A99ManagerInputField>();
        ...
        for (int i = 0; i < _inputFields.Count; i++)
        {
            for (int j = 0; j < temporary.Count; j++)
            {
                if (temporary.Contains(_inputFields[i].text))
                {
                    ...
                    {
                        _inputFields[i].GetComponent<M3A99BehaviourInputField>()._isRight = true;
                ...
                {
                    _inputFields[i].GetComponent<M3A99BehaviourInputField>()._isRight = false;
                    break;
                }
