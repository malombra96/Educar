...
public class M3A99ManagerDragDrop : MonoBehaviour
{
    ...
    public List<M3A99ManagerDragDrop> managerDragDrops;
    public List<string> nombresDrags;

    [Header("Item List")]
    /*[HideInInspector]*/ public List<M3A99BehaviourDrag> _drags;
    [HideInInspector] public List<M3A99BehaviourDrop> _drops;

    [HideInInspector] public List<M3A99BehaviourDropGroup> _groups;
    ...
    void OnEnable()
    {
        ...
        if (gameObject.activeSelf)
        {
            ...
            {
                var drag = transform.GetChild(x).gameObject;
                foreach (var nombreDrag in nombresDrags)
                {
                    if (nombreDrag == drag.name)
                    {
    ...
    IEnumerator RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<M3A99BehaviourDrag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
        {
            ...
            drag.GetComponent<M3A99BehaviourDrag>().enabled = false;

            if (drag._drop)                           
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<M3A99BehaviourDrop>())));
            
            if (drag._DropRight.Count > 0 && (d !=drag.GetComponent<Image>().sprite || !d))
            {
                print(drag.name);
                l++;
                foreach (var managerDragDrop in managerDragDrops)
                {
                    if (l <= 2)
                        managerDragDrop.nombresDrags.Add(drag.name);

                }
                d = drag.GetComponent<Image>().sprite;
            }

        }
        foreach (var drop in _drops)
            if (drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<M3A99BehaviourDrag>()._DropRight.Contains(drop)));

        foreach (var answer in answers)
        {
            var BhDrop = answer.Key.GetComponent<M3A99BehaviourDrop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<M3A99BehaviourDropGroup>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<M3A99BehaviourDropGroup>()._currentWrong++;
                }
            }
    ...
    public void ImmediatelyValidation(M3A99BehaviourDrag drag, M3A99BehaviourDrop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M3A99BehaviourDrag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<BehaviourDrop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<BehaviourDrag>())
    ...
    public void SetPuntaje()
    {
        ...
        foreach (var answer in answers)
        {
            if (answer.Key.GetComponent<M3A99BehaviourDrag>() && answer.Value)
                rights++;
        }
    ...
    public void ResetDragDrop()
    {
        ...
        {         
            ...
            if ((_OperatingMethod == M3A99ManagerDragDrop.OperatingMethod.Create) && !drag.GetComponent<M3A99BehaviourDrag>().ItsInfinite)
                DestroyImmediate(drag.gameObject);
        }

        for (int x = 0; x < transform.childCount; x++)
        {
            if (transform.GetChild(x).GetComponent<M3A99BehaviourDrag>() && !transform.GetChild(x).gameObject.activeSelf)
                transform.GetChild(x).gameObject.SetActive(true);
        }

        if (_OperatingMethod == M3A99ManagerDragDrop.OperatingMethod.Create)
