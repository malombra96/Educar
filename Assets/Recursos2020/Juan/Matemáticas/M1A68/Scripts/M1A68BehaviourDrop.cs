...
public class M1A68BehaviourDrop : MonoBehaviour,IDropHandler
{    
   /* [HideInInspector]  */ [Header("Drag-IN")] public GameObject _drag;
    [Header("Grupo")] public M1A68BehaviourDropGroup _group;
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        if (x != null && _drag == null)
        {
            ...
            if (_drag.GetComponent<M1A68BehaviourDrag>())
            {
                UpdateSlotDrop();
                ...
                _drag.GetComponent<M1A68BehaviourDrag>().inDrop = true;
                UpdateRowDrag(_drag);
                GetCalificationType();
            }
            else
    ...
    void UpdateSlotDrop()
    {
        if (_drag.GetComponent<M1A68BehaviourDrag>()._drop == null)
        {
            _drag.GetComponent<M1A68BehaviourDrag>()._drop = gameObject;
        }
        else
        {
            var previousDrop = _drag.GetComponent<M1A68BehaviourDrag>()._drop.GetComponent<M1A68BehaviourDrop>();
    ...
    void UpdateRowDrag(GameObject other)
    {
        if(_managerDragDrop._OperatingMethod == M1A68ManagerDragDrop.OperatingMethod.Match)
        {
            ...
            Vector2 def = other.GetComponent<M1A68BehaviourDrag>()._defaultPos;
            other.GetComponent<M1A68BehaviourDrag>()._row = this._row;

            foreach (var drag in _managerDragDrop._drags)
            {
                if (drag._row == other.GetComponent<M1A68BehaviourDrag>()._row && drag.gameObject != other)
                {
                    drag._row = x;
                    drag.GetComponent<RectTransform>().anchoredPosition = def;
                    drag.GetComponent<M1A68BehaviourDrag>().SwapDefaultPosition(other);
                    drag.UpdateCurrentPosition();
                    other.GetComponent<M1A68BehaviourDrag>().UpdateCurrentPosition();
                }
            }
        }
    ...
    {
        if (_managerDragDrop._TypeValidation == M1A68ManagerDragDrop.TypeValidation.Inmediata)
            _managerDragDrop.ImmediatelyValidation(_drag.GetComponent<M1A68BehaviourDrag>(),this);
        else
