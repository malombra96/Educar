...
public class M1A68ManagerDragDrop : MonoBehaviour
{
    ...
    [Header("Item List")]
    [HideInInspector] public List<M1A68BehaviourDrag> _drags;
    [HideInInspector] public List<M1A68BehaviourDrop> _drops;

    [HideInInspector] public List<M1A68BehaviourDropGroup> _groups;
    ...
    IEnumerator RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<M1A68BehaviourDrag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
            if (drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<M1A68BehaviourDrop>())));


        foreach (var drop in _drops)
            if (drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<M1A68BehaviourDrag>()._DropRight.Contains(drop)));

        foreach (var answer in answers)
        {
            var BhDrop = answer.Key.GetComponent<M1A68BehaviourDrop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<M1A68BehaviourDropGroup>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<M1A68BehaviourDropGroup>()._currentWrong++;
                }
            }
    ...
    public void ImmediatelyValidation(M1A68BehaviourDrag drag, M1A68BehaviourDrop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M1A68BehaviourDrag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M1A68BehaviourDrop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    public void TypeCalificationGroup()
    {
        foreach (var group in _groups)
        {
            ...
            SetSpriteAnswer(group.transform.GetChild(0).GetComponent<Image>(), (group._needRight == group._currentRight && group._currentWrong == 0));
        }                
    ...
    public void SetPuntaje()
    {
        ...
        foreach (var answer in answers)
        {
            M1A68BehaviourDrag temp = answer.Key.GetComponent<M1A68BehaviourDrag>();
    ...
    public void ResetDragDrop()
    {
        ...
        foreach (var group in _groups)
        {
            ...
            group.transform.GetChild(0).gameObject.SetActive(false);
