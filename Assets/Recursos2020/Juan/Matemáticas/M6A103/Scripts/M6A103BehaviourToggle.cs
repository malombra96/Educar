...
public class M6A103BehaviourToggle : MonoBehaviour
{
    ControlAudio _controlAudio;
    Toggle _toggle;
    [Header("is Right?")] public bool isRight;
    M6A103BehaviourToggleGroup _behaviourToggleGroup;
    M6A103ManagerSeleccionarToggle _managerToggle;

    void Start()
    {
        _controlAudio = FindObjectOfType<ControlAudio>();
        _toggle = GetComponent<Toggle>();
        _behaviourToggleGroup = transform.parent.GetComponent<M6A103BehaviourToggleGroup>();
        _managerToggle = transform.parent.parent.GetComponent<M6A103ManagerSeleccionarToggle>();
        _toggle.onValueChanged.AddListener(delegate { SetStateToggle(); });

        if (_managerToggle._NeedSymbol == M6A103ManagerSeleccionarToggle.NeedSymbol.withSymbol)
    ...
    void SetStateToggle()
    {
        ...
        {
            case M6A103ManagerSeleccionarToggle.TypeSelection.onexgroup:
                ...
                if (_toggle.isOn)
                {
                    ...
                    switch (_managerToggle._typeVal)
                    {
                        ...
                        case M6A103ManagerSeleccionarToggle.TypeValidation.inmediata:
            ...
            case M6A103ManagerSeleccionarToggle.TypeSelection.variousxgroup:
