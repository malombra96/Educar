...
public class M6A103BehaviourGroupInputField : MonoBehaviour
{
    M6A103ManagerInputField _managerInputField;
    ...
    void Awake()
    {
        _managerInputField = FindObjectOfType<M6A103ManagerInputField>();
        ...
        for (int i = 0; i < _inputFields.Count; i++)
        {
            for (int j = 0; j < temporary.Count; j++)
            {
                if (temporary.Contains(_inputFields[i].text))
                {
                    ...
                    {
                        _inputFields[i].GetComponent<M6A103BehaviourInputField>()._isRight = true;
                ...
                {
                    _inputFields[i].GetComponent<M6A103BehaviourInputField>()._isRight = false;
                    break;
                }
