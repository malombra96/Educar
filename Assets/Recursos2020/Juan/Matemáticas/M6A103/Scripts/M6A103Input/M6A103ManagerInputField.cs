...
public class M6A103ManagerInputField : MonoBehaviour
{
    public int pregunta;
    public GameObject animacion;
    public M6A103ManagerInputField managerInputField;
    public bool sinRepetir;
    ...
    [HideInInspector] public List<M6A103BehaviourGroupInputField> _groupInputField;
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M6A103BehaviourInputField>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    public void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M6A103BehaviourInputField>()._isRight = false;
                inputField.GetComponent<M6A103BehaviourInputField>()._isEnabled = true;
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == M6A103BehaviourGroupInputField.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<M6A103BehaviourInputField>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<M6A103BehaviourInputField>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
                    ...
                    if (sinRepetir)
                    {
                        if (j > 0)
                        {
                            if (f.GetComponent<InputField>().text == _groupInputField[i]._inputFields[j - 1].text)
                            {
                                _listAnswers[i].answers[j] = false;
                                answer = false;
                            }
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<M6A103BehaviourInputField>()._isRight;
        ...
        if (_TipoPuntuacion == TipoPuntuacion.grupo)
        {
            ...
            if (groupTemp == 1) 
            {
                ...
                if (managerInputField)
                    managerInputField.pregunta = pregunta;
            }
            if (pregunta == 5)
