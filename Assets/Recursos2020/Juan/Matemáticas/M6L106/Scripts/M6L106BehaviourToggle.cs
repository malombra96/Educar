...
public class M6L106BehaviourToggle : MonoBehaviour
{
    ControlAudio _controlAudio;
    Toggle _toggle;
    [Header("is Right?")] public bool isRight;
    M6L106BehaviourToggleGroup _behaviourToggleGroup;
    M6L106ManagerSeleccionarToggle _managerToggle;

    void Start()
    {
        _controlAudio = FindObjectOfType<ControlAudio>();
        _toggle = GetComponent<Toggle>();
        _behaviourToggleGroup = transform.parent.GetComponent<M6L106BehaviourToggleGroup>();
        _managerToggle = transform.parent.parent.GetComponent<M6L106ManagerSeleccionarToggle>();
        _toggle.onValueChanged.AddListener(delegate { SetStateToggle(); });

        if (_managerToggle._NeedSymbol == M6L106ManagerSeleccionarToggle.NeedSymbol.withSymbol)
        ...
        switch (_managerToggle._TypeSelect)
        {
            case M6L106ManagerSeleccionarToggle.TypeSelection.onexgroup:
                ...
                if (_toggle.isOn)
                {
                    ...
                    SetSpriteToggle(_managerToggle.intentos.transform.GetChild(_managerToggle.intento).GetComponent<Image>(), _toggle.isOn);
                    ...
                    switch (_managerToggle._typeVal)
                    {
                        ...
                        case M6L106ManagerSeleccionarToggle.TypeValidation.inmediata:
