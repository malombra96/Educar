...
public class M6L106ManagerInputField : MonoBehaviour
{
    public int puntos;
    public M6L106ManagerInputField[] managerInputField;
    ...
    [HideInInspector] public List<M6L106BehaviourGroupInputField> _groupInputField;
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M6L106BehaviourInputField>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    public void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M6L106BehaviourInputField>()._isRight = false;
                inputField.GetComponent<M6L106BehaviourInputField>()._isEnabled = true;
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == M6L106BehaviourGroupInputField.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<M6L106BehaviourInputField>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<M6L106BehaviourInputField>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<M6L106BehaviourInputField>()._isRight;
    ...
    private void OnEnable()
    {
        ...
        //     animacion.SetBool("pregunta " + Convert.ToString(x), true);
        if(puntos>0)
           animacion.SetBool("pregunta " + Convert.ToString(puntos+1), true);
        StartCoroutine(pausa(0.01f));
    }
   
    void ajustarAnimacion(bool j)
    {          
        if (j)
        {
            puntos++;
            animacion.speed = 1;
            animacion.SetBool("pregunta " + Convert.ToString(puntos), true);

            for (int x = 0; x < managerInputField.Length; x++)
                managerInputField[x].animacion.SetBool("pregunta " + Convert.ToString(puntos), true);

            StartCoroutine(pausa(1f));
        }
        for (int x = 0; x < managerInputField.Length; x++)
            managerInputField[x].puntos = puntos;
        //managerInputField(x).puntos = puntos;
    }
    IEnumerator pausa(float t)
    {
    ...
    public void resetAll()
    {
        ...
        puntos = 0;
        for (int x = 1; x <= 5; x++)
            animacion.SetBool("pregunta " + Convert.ToString(x), false);
        InitializeState();
    }
}
