...
public class M1L71ManagerDragDrop : MonoBehaviour
{
    int x = 0;
    ...
    [Header("Item List")]
    [HideInInspector] public List<M1L71BehaviourDrag> _drags;
    /*[HideInInspector]*/ public List<M1L71BehaviourDrop> _drops;
    ...
    public IEnumerator StateBtnValidar()
    {
        ...
        if (_customValidar == 0)
            _validar.interactable = (i == _drops.Count);
        else
    ...
    IEnumerator RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<M1L71BehaviourDrag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
            if (drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<M1L71BehaviourDrop>())));


        foreach (var drop in _drops)
            if (drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<M1L71BehaviourDrag>()._DropRight.Contains(drop)));        
    ...
    public void ImmediatelyValidation(M1L71BehaviourDrag drag, M1L71BehaviourDrop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M1L71BehaviourDrag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M1L71BehaviourDrop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    public void SetPuntaje()
    {
        ...
        foreach (var answer in answers)
        {
            M1L71BehaviourDrag temp = answer.Key.GetComponent<M1L71BehaviourDrag>();
        ...
        if (animacio && rights == _drops.Count)
        {
            for (int i = 0; i < _drags.Count; i++) 
            {
                if (_drags[i].GetComponent<M1L71BehaviourDrag>()._drop != null)
                {
                    x = i;
                }
            }
            _drags[x].GetComponent<Animator>().enabled = true;
            _drags[x].transform.GetChild(0).gameObject.SetActive(true);
            _drops[0].GetComponent<Animator>().SetBool("correcto", true);
            _drags[x].GetComponent<Animator>().SetBool("correcto", true);
            
            _controlNavegacion.Forward(2.30f);
        }
        else
        {
            _controlNavegacion.Forward(2);
    ...
    public void ResetDragDrop()
    {
        ...
        if (animacio)
        {
            _drops[0].GetComponent<Animator>().SetBool("correcto", false);
            _drags[x].GetComponent<Animator>().SetBool("correcto", false);
            _drags[x].GetComponent<Animator>().enabled = false;
            _drags[x].transform.GetChild(0).gameObject.SetActive(true);
        }
        foreach (var drag in _drags)
        {
