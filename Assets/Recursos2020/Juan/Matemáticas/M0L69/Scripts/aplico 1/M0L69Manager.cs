...
public class M0L69Manager : MonoBehaviour
{
    ControlPuntaje _controlPuntaje;
    ControlNavegacion _controlNavegacion;
    public GameObject SegundaOportundad;
    [HideInInspector] public ControlAudio _controlAudio;
    [HideInInspector] public Canvas canvas;
    int intentos;
    [Header("Item List")]
    [HideInInspector] public List<M0L69Drag> _drags;
    [HideInInspector] public List<M0L69Drop> _drops;

    [HideInInspector] public List<M0L69DropGroup> _groups;
    ...
    #endregion
    public M0L69Manager manager; //esto es para eliminar los dags ya seleccionados;
    public List<GameObject> dragsDesavilitar;
    void Awake()
    {
    ...
    IEnumerator RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<M0L69Drag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
            if (drag._drop)
            {
                if (manager)
                {
                    manager.dragsDesavilitar.Add(drag.gameObject);
                }
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<M0L69Drop>())));
            }
       

        foreach (var drop in _drops)
            if (drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<M0L69Drag>()._DropRight.Contains(drop)));

        foreach (var answer in answers)
        {
            var BhDrop = answer.Key.GetComponent<M0L69Drop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<M0L69DropGroup>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<M0L69DropGroup>()._currentWrong++;
                }
            }
    ...
    public void ImmediatelyValidation(M0L69Drag drag, M0L69Drop drop)
    {
    ...
    public void TypeCalificationDrag()
    {
        foreach (var answer in answers)
        {            
            if (answer.Key.GetComponent<M0L69Drag>())
            {                
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
            }
        }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M0L69Drop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    public void SetPuntaje()
    {
        ...
        foreach (var answer in answers)
        {
            M0L69Drag temp = answer.Key.GetComponent<M0L69Drag>();
    ...
    {        
        ...
        if (manager && intentos == 2   || rights == 2 && manager)
        {
            for (int x = 0; x < manager.transform.childCount; x++)
            {
                foreach (var i in manager.dragsDesavilitar)
                {
                    if (manager.transform.GetChild(x).name == i.name)
                    {
                        manager.transform.GetChild(x).gameObject.SetActive(false);                        
                    }
    ...
    IEnumerator activar()
    {
        ...
        double temp = Convert.ToDouble(rights);
        _controlPuntaje.IncreaseScore(Convert.ToInt32(_controlPuntaje._rightAnswers -= temp));
        activarDrags = false;
        ResetDragDrop();
        SegundaOportundad.transform.SetAsLastSibling();
        SegundaOportundad.SetActive(true);
        intentos++;
    ...
    {        
        SegundaOportundad.SetActive(false);
        rights = 0;
        intentos = 0;
        if (manager)
        {               
            manager.dragsDesavilitar.Clear();
        }

        for (int i = 0; i < gameObject.transform.childCount; i++)
        {
            if (activarDrags && !gameObject.transform.GetChild(i).gameObject.activeSelf && gameObject.transform.GetChild(i).GetComponent<M0L69Drag>())
            {
