...
public class M0A65ManagerDragDrop : MonoBehaviour
{
    public Image niño;
    ...
    [Header("Item List")]
    /*[HideInInspector]*/ public List<M0A65BehaviourDrag> _drags;
    [HideInInspector] public List<M0A65BehaviourDrop> _drops;

    [HideInInspector] public List<M0A65BehaviourDropGroup> _groups;
    ...
    IEnumerator RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<M0A65BehaviourDrag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
            if (drag._drop && drag)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<M0A65BehaviourDrop>())));


        foreach (var drop in _drops)
            if (drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<M0A65BehaviourDrag>()._DropRight.Contains(drop)));

        foreach (var answer in answers)
        {
            var BhDrop = answer.Key.GetComponent<M0A65BehaviourDrop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<M0A65BehaviourDropGroup>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<M0A65BehaviourDropGroup>()._currentWrong++;
                }
            }
    ...
    public void ImmediatelyValidation(M0A65BehaviourDrag drag, M0A65BehaviourDrop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M0A65BehaviourDrag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M0A65BehaviourDrop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    public void TypeCalificationGroup()
    {
        ...
        foreach (var group in _groups)
        {
            ...
            SetSpriteAnswer(niño.GetComponent<Image>(), (group._needRight == group._currentRight && o == _groups.Count && group.j == group._currentRight));
        }
    }
    ...
    public void SetPuntaje()
    {
        ...
        foreach (var answer in answers)
        {
            M0A65BehaviourDrag temp = answer.Key.GetComponent<M0A65BehaviourDrag>();
    ...
    public void SetPuntajeGroups()
    {
        ...
        _controlNavegacion.Forward(2);
        _controlAudio.PlayAudio((i == 1) ? 1 : 2);
        _controlPuntaje.IncreaseScore(i);
    }
    ...
    public void ResetDragDrop()
    {
      ...
      niño.GetComponent<Image>().sprite = niño.GetComponent<BehaviourSprite>()._default;
      niño.GetComponent<Image>().SetNativeSize();
        ...
        foreach (var group in _groups)
        {
            ...
            group.GetComponent<M0A65BehaviourDropGroup>().j= 0 ;
            group._currentRight = 0;
            group._currentWrong = 0;       

            for(int x = 0; x < group.transform.childCount; x++)
            {
                Destroy(group.transform.GetChild(x).GetComponent<M0A65BehaviourDrop>()._drag);
                group.transform.GetChild(x).GetComponent<M0A65BehaviourDrop>()._drag = null;
            }
        }           
        _drags.Clear();
        _drags.Add(transform.GetChild(7).GetComponent<M0A65BehaviourDrag>());        
        _drags.Add(transform.GetChild(8).GetComponent<M0A65BehaviourDrag>());       
        answers.Clear();        
