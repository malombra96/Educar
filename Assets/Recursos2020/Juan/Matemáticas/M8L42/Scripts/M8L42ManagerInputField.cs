...
public class M8L42ManagerInputField : MonoBehaviour
{
    public Animator carrito;
    ...
    /*[HideInInspector]*/ public List<M8L42BehaviourGroupInputField> _groupInputField;
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M8L42BehaviourInputField>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    public void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M8L42BehaviourInputField>()._isRight = false;
                inputField.GetComponent<M8L42BehaviourInputField>()._isEnabled = true;
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == M8L42BehaviourGroupInputField.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<M8L42BehaviourInputField>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<M8L42BehaviourInputField>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<M8L42BehaviourInputField>()._isRight;
        ...
        carrito.SetBool("correcto", groupTemp == _groupInputField.Count);
        _controlNavegacion.Forward(groupTemp == _groupInputField.Count ? 3 : 2);
    ...
    public void resetAll()
    {
        ...
        carrito.SetBool("correcto", false);
        InitializeState();
    }
}
