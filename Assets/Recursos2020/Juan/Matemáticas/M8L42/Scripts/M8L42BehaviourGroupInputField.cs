...
public class M8L42BehaviourGroupInputField : MonoBehaviour
{
    M8L42ManagerInputField _managerInputField;
    ...
    void Awake()
    {
        _managerInputField = FindObjectOfType<M8L42ManagerInputField>();        
        ...
        for (int i = 0; i < _inputFields.Count; i++)
        {
            for (int j = 0; j < temporary.Count; j++)
            {
                if (temporary.Contains(_inputFields[i].text))
                {
                    ...
                    {
                        _inputFields[i].GetComponent<M8L42BehaviourInputField>()._isRight = true;
                ...
                {
                    _inputFields[i].GetComponent<M8L42BehaviourInputField>()._isRight = false;
                    break;
                }
