...
public class M6L105BehaviourToggle : MonoBehaviour
{
    ControlAudio _controlAudio;
    Toggle _toggle;
    [Header("is Right?")] public bool isRight;
    M6L105BehaviourToggleGroup _behaviourToggleGroup;
    M6L105ManagerSeleccionarToggle _managerToggle;

    void Start()
    {
        _controlAudio = FindObjectOfType<ControlAudio>();
        _toggle = GetComponent<Toggle>();
        _behaviourToggleGroup = transform.parent.GetComponent<M6L105BehaviourToggleGroup>();
        _managerToggle = transform.parent.parent.GetComponent<M6L105ManagerSeleccionarToggle>();
        _toggle.onValueChanged.AddListener(delegate { SetStateToggle(); });

        if (_managerToggle._NeedSymbol == M6L105ManagerSeleccionarToggle.NeedSymbol.withSymbol)
            ...
            case M6L105ManagerSeleccionarToggle.TypeSelection.onexgroup:
                ...
                if (_toggle.isOn)
                {
                    ...
                    switch (_managerToggle._typeVal)
                    {
                        ...
                        case M6L105ManagerSeleccionarToggle.TypeValidation.inmediata:
