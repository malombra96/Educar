...
public class M9L59BehaviourGroupInputField : MonoBehaviour
{
    M9L59ManagerInputField _managerInputField;
    ...
    void Awake()
    {
        _managerInputField = FindObjectOfType<M9L59ManagerInputField>();
        ...
        for (int i = 0; i < _inputFields.Count; i++)
        {
            for (int j = 0; j < temporary.Count; j++)
            {
                if (temporary.Contains(_inputFields[i].text))
                {
                    ...
                    {
                        _inputFields[i].GetComponent<M9L59BehaviourInputField>()._isRight = true;
                ...
                {
                    _inputFields[i].GetComponent<M9L59BehaviourInputField>()._isRight = false;
                    break;
                }
