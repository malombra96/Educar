...
public class M9L59ManagerDragDrop : MonoBehaviour
{
    ...
    [Header("Item List")]
    /* [HideInInspector] */
    public List<M9L59BehaviourDrag> _drags;
    [HideInInspector] public List<M9L59BehaviourDrop> _drops;

    [HideInInspector] public List<M9L59BehaviourDropGroup> _groups;
    ...
    IEnumerator RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<M9L59BehaviourDrag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
        {
            drag.GetComponent<M9L59BehaviourDrag>().enabled = false;

            if (drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<M9L59BehaviourDrop>())));
        }
        ...
        foreach (var drop in _drops)
            if (drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<M9L59BehaviourDrag>()._DropRight.Contains(drop)));

        foreach (var answer in answers)
        {
            var BhDrop = answer.Key.GetComponent<M9L59BehaviourDrop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<M9L59BehaviourDropGroup>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<M9L59BehaviourDropGroup>()._currentWrong++;
                }
            }
    ...
    public void ImmediatelyValidation(M9L59BehaviourDrag drag, M9L59BehaviourDrop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M9L59BehaviourDrag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M9L59BehaviourDrop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    public void SetPuntaje()
    {
        ...
        foreach (var answer in answers)
        {
            if (answer.Key.GetComponent<M9L59BehaviourDrag>() && answer.Value)
                rights++;
        }
    ...
    public void ResetDragDrop()
    {
        ...
        foreach (var drag in _drags)
        {
            ...
            if ((_OperatingMethod == M9L59ManagerDragDrop.OperatingMethod.Create) && !drag.GetComponent<M9L59BehaviourDrag>().ItsInfinite)
                DestroyImmediate(drag.gameObject);
        }

        if (_OperatingMethod == M9L59ManagerDragDrop.OperatingMethod.Create)
