...
public class M10A8ManagerSeleccionarToggle : MonoBehaviour
{
    ...
    [HideInInspector] public List<M10A8BehaviourToggleGroup> _groupToggle;
    ...
    void EvaluateGroupsToggle()
    {
        ...
        for (int j = 0; j < _groupToggle.Count; j++)
        {
            ...
            for (int i = 0; i < _groupToggle[j].transform.childCount; i++)
            {
                ...
                bool state = _groupToggle[j].transform.GetChild(i).GetComponent<M10A8BehaviourToggle>().isRight;
    ...
    public void EvaluateGroupsToggleImmediately(M10A8BehaviourToggleGroup completed)
    {
        bool answer = false;
        ...
        for (int i = 0; i < _groupToggle[indexGroup].transform.childCount; i++)
        {
            ...
            bool state = _groupToggle[indexGroup].transform.GetChild(i).GetComponent<M10A8BehaviourToggle>().isRight;
        ...
        answer = (t.isOn && t.GetComponent<M10A8BehaviourToggle>().isRight);
    ...
    void SetPuntaje()
    {
        ...
        { 
            for(int i = 0; i < _groupToggle[j].transform.childCount; i++)
            {
                var toggle = _groupToggle[j].transform.GetChild(i).GetComponent<Toggle>();
                if (toggle.isOn && toggle.transform.childCount > 0)
    ...
    {
        if (_NeedSymbol == M10A8ManagerSeleccionarToggle.NeedSymbol.onlyToggle)
        {
            i.sprite = state ?
    ...
    void SetDefaultSprite(Image i)
    {
        ...
        if (_NeedSymbol == M10A8ManagerSeleccionarToggle.NeedSymbol.withSymbol)
    ...
    public void ResetSeleccionarToggle()
    {
        ...
        for (int j = 0; j < _groupToggle.Count; j++)
        {
            for (int i = 0; i < _groupToggle[j].transform.childCount; i++)
            {
                ...
                if (_groupToggle[j].transform.GetChild(i).transform.childCount > 0)
                    _groupToggle[j].transform.GetChild(i).transform.GetChild(0).gameObject.SetActive(false);
                Image img = _groupToggle[j].transform.GetChild(i).GetComponent<Image>();
