...
public class M6L108ManagerDragDrop : MonoBehaviour
{
    ...
    [Header("Item List")]
    /* [HideInInspector] */
    public List<M6L108BehaviourDrag> _drags;
    [HideInInspector] public List<M6L108BehaviourDrop> _drops;

    [HideInInspector] public List<M6L108BehaviourDropGroup> _groups;
    ...
    IEnumerator RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<M6L108BehaviourDrag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
        {
            drag.GetComponent<M6L108BehaviourDrag>().enabled = false;

            if (drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<M6L108BehaviourDrop>())));
        }

        foreach (var drop in _drops)
            if (drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<M6L108BehaviourDrag>()._DropRight.Contains(drop)));

        foreach (var answer in answers)
        {
            var BhDrop = answer.Key.GetComponent<M6L108BehaviourDrop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<M6L108BehaviourDropGroup>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<M6L108BehaviourDropGroup>()._currentWrong++;
                }
            }
    ...
    public void ImmediatelyValidation(M6L108BehaviourDrag drag, M6L108BehaviourDrop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M6L108BehaviourDrag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M6L108BehaviourDrop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    public void SetPuntaje()
    {
        ...
        foreach (var answer in answers)
        {
            if (answer.Key.GetComponent<M6L108BehaviourDrag>() && answer.Value)
                rights++;
        }
