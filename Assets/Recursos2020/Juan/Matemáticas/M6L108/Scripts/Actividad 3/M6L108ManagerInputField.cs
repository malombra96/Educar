...
public class M6L108ManagerInputField : MonoBehaviour
{
    ...
    [HideInInspector] public List<M6L108BehaviourGroupInputField> _groupInputField;
    ...
    void Start()
    {
        ...
        botonValidar.onClick.AddListener(delegate { StartCoroutine(CalificarInputs()); });
    }
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M6L108BehaviourInputField>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    public void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M6L108BehaviourInputField>()._isRight = false;
                inputField.GetComponent<M6L108BehaviourInputField>()._isEnabled = true;
    ...
    public IEnumerator CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == M6L108BehaviourGroupInputField.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<M6L108BehaviourInputField>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<M6L108BehaviourInputField>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<M6L108BehaviourInputField>()._isRight;
        ...
        animator.Play(groupTemp == _groupInputField.Count ? "correcto" : "error");
