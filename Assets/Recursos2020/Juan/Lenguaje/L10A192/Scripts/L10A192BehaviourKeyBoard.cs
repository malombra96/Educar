...
public class L10A192BehaviourKeyBoard : MonoBehaviour
{
    ControlAudio _controlAudio;

    [Header("Input Texto")] public InputField _InputName;
    [HideInInspector] public string _name; // name
    private bool _firstWord = true; // State first word
    [HideInInspector] public int _contadorWords; // Contador de letras ingresadas
    ...
    private void Start()
    {
        ...
        cambion.onClick.AddListener(Cambio);
        /// Inicializa variables locales y desactiva el boton enter
        _name = "";
        _contadorWords = 0;
        _save.onClick.AddListener(desactivarTeclado);
    ...
    void desactivarTeclado()
    {
        ...
        cambion.transform.GetChild(0).GetComponent<Text>().text = "ABC";
    ...
    void Cambio()
    {
        _controlAudio.PlayAudio(0);
        string c = "";
        if (transform.GetChild(0).transform.GetChild(0).gameObject.activeSelf)
        {
        ...
        cambion.transform.GetChild(0).GetComponent<Text>().text = c;
    }
