...
public class L10A192ManagerDragDrop : MonoBehaviour
{
    ...
    [Header("Item List")]
    /* [HideInInspector] */
    public List<L10A192BehaviourDrag> _drags;
    [HideInInspector] public List<L10A192BehaviourDrop> _drops;    
    ...
    IEnumerator RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<L10A192BehaviourDrag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
        {
            drag.GetComponent<L10A192BehaviourDrag>().enabled = false;

            if (drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<L10A192BehaviourDrop>())));
        }
        ...
        foreach (var drop in _drops)
            if (drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<L10A192BehaviourDrag>()._DropRight.Contains(drop)));

        //foreach (var answer in answers)
        //{
        //    var BhDrop = answer.Key.GetComponent<L10A192BehaviourDrop>();
    ...
    public void ImmediatelyValidation(L10A192BehaviourDrag drag, L10A192BehaviourDrop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<L10A192BehaviourDrag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<L10A192BehaviourDrop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    public void SetPuntaje()
    {
        ...
        foreach (var answer in answers)
        {
            if (answer.Key.GetComponent<L10A192BehaviourDrag>() && answer.Value)
                rights++;
        }
    ...
    public void ResetDragDrop()
    {
        ...
        foreach (var drag in _drags)
        {
            ...
            drag.padre = GetComponent<Transform>();
            drag.enabled = true;
            drag._drop = null;
