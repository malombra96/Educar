...
public class L3A232BehaviourInputField : MonoBehaviour, IPointerClickHandler
{
    L3A232ManagerInputField _managerInputField;
    InputField _input;
    ...
    void Awake()
    {
        ...
        _managerInputField = FindObjectOfType<L3A232ManagerInputField>();
    ...
    void Update()
    {
        if (Text)
        {
            Text.gameObject.SetActive(_managerInputField._lightBox.activeSelf);
            if (!Text.gameObject.activeSelf)
                Text = null;
        }
        if (!Application.isMobilePlatform && Text && Text.gameObject.activeSelf && texto)
    ...
    public void OnPointerClick(PointerEventData pointerEventData)
    {
        if (!Application.isMobilePlatform && _isEnabled)
        {
            ...
            L3A232BehaviourKeyBoard _managerDisplay = _managerInputField._lightBox.GetComponent<L3A232BehaviourKeyBoard>();            
            _managerInputField.desactivar(GetComponent<InputField>());
            if (Text)
            {                  
                _managerDisplay._InputName = Text;
                _managerDisplay._limitCharacter = Text.characterLimit;
                if (!texto)
                {
                    _managerDisplay._name = "";
            ...
            //else
            //{
            //    _managerDisplay._InputName = GetComponent<InputField>();
