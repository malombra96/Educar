...
public class L3A232ManagerInputField : MonoBehaviour
{
    ...
    [HideInInspector] public List<L3A232BehaviourGroupInputField> _groupInputField;
    ...
    void OnDisable()=> _lightBox.SetActive(false);
    public void desactivar(InputField i)
    {
        foreach(var g in _groupInputField)
        {
            for (int x = 0; x < g.transform.childCount; x++)
                g.transform.GetChild(x).GetComponent<L3A232BehaviourInputField>().Text = null;
        }
        i.GetComponent<L3A232BehaviourInputField>().Text = transform.GetChild(transform.childCount - 1).GetComponent<InputField>();
        transform.GetChild(transform.childCount - 1).GetComponent<InputField>().text = i.GetComponent<L3A232BehaviourInputField>().Text.text;
        _lightBox.GetComponent<L3A232BehaviourKeyBoard>()._name = i.text;
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<L3A232BehaviourInputField>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    public void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<L3A232BehaviourInputField>()._isRight = false;
                inputField.GetComponent<L3A232BehaviourInputField>()._isEnabled = true;
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == L3A232BehaviourGroupInputField.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<L3A232BehaviourInputField>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<L3A232BehaviourInputField>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<L3A232BehaviourInputField>()._isRight;
