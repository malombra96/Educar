...
public class L10A190_letra : MonoBehaviour
{
    private Text _text;
    ...
    IEnumerator DelayKey(GameObject button)
    {
        ...
        {
            //transform.parent.parent.gameObject.GetComponent<L4A238_k>()._animator.enabled = true;
            transform.parent.parent.gameObject.SetActive(false);/*GetComponent<L4A238_k>()._animator.Play("KeyBoard_out");// Desactivar Keypad*/
            //transform.parent.parent.gameObject.GetComponent<L4A238_k>()._animator.enabled = true;
            gameObject.GetComponent<Image>().color = new Color32(237, 120, 7, 255);
            transform.GetChild(0).gameObject.GetComponent<Image>().color = Color.white;
