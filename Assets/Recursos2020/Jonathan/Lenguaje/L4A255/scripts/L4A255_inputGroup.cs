...
public class L4A255_inputGroup : MonoBehaviour
{
        ...
        for (int i = 0; i < _inputFields.Count; i++)
        {
            for (int j = 0; j < temporary.Count; j++)
            {
                if (temporary.Contains(_inputFields[i].text))
                {
                    ...
                    {
                        _inputFields[i].GetComponent<L4A255_input>()._isRight = true;
                ...
                {
                    _inputFields[i].GetComponent<L4A255_input>()._isRight = false;
                    break;
                }
