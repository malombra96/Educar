...
public class L4A255_managerInput : MonoBehaviour
{
    public GameObject siguiente,elementos,info;
    ...
    [HideInInspector] public List<L4A255_inputGroup> _groupInputField;
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<L4A255_input>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    public void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<L4A255_input>()._isRight = false;
                inputField.GetComponent<L4A255_input>()._isEnabled = true;
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == L4A255_inputGroup.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<L4A255_input>()._isEnabled = false;

                    bool answer;
                    //answer = (f.GetComponent<L4A255_input>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
                    answer = (f.GetComponent<InputField>().text.Length>0);
                    f.GetComponent<L4A255_input>()._isRight = answer;
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<L4A255_input>()._isRight;
