...
public class L4A237_managerDrag : MonoBehaviour
{
    public GameObject grupo,validar;
    public List<GameObject> drops, drags;
    public GameObject nadador, bg;
    ...
    [Header("Item List")]
    public List<L4A237_drag> _drags;
    public List<L4A237_drop> _drops;

    public List<L4A237_groupDrag> _groups;
    ...
    void RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<L4A237_drag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
            if (drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<L4A237_drop>())));
        ...
        {
            var BhDrop = answer.Key.GetComponent<L4A237_drop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<L4A237_groupDrag>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<L4A237_groupDrag>()._currentWrong++;
                }
            }
            //print(answer.Key.GetComponent<L4A237_drop>()._group+""+answer.Key + answer);
        }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<L4A237_drag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<L4A237_drop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
