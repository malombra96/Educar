...
public class L11L249_randomLine : MonoBehaviour
{
    ...
    private void Start()
    {
        for (int j = 0; j < _TargetsList.Count; j++)
        {
            _TargetsList[j].GetComponent<BehaviourDrawToggle>()._point = _TargetsList[j].GetComponent<RectTransform>().position;
            _TargetsList[j].GetComponent<BehaviourDrawToggle>()._point.z = -9.6f;
        }
    }
}
