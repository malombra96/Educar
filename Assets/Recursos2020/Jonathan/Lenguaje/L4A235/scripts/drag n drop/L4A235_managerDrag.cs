...
public class L4A235_managerDrag : MonoBehaviour
{
    public L4A235_managerGeneral general;
    public bool first;
    ...
    [Header("Item List")]
    public List<L4A235_drag> _drags;
    public List<L4A235_drop> _drops;

    public List<L4A235_groupDrop> _groups;
    ...
    void RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<L4A235_drag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
            if (drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<L4A235_drop>())));


        foreach (var drop in _drops)
            if (drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<L4A235_drag>()._DropRight.Contains(drop)));

        foreach (var answer in answers)
        {
            var BhDrop = answer.Key.GetComponent<L4A235_drop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<L4A235_groupDrop>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<L4A235_groupDrop>()._currentWrong++;
                }
            }
            //print(answer.Key.GetComponent<L4A235_drop>()._group+""+answer.Key + answer);
        }
    ...
    public void ImmediatelyValidation(L4A235_drag drag, L4A235_drop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<L4A235_drag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<L4A235_drop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    public void SetPuntaje()
    {
        ...
        foreach (var answer in answers)
        {
            L4A235_drag temp = answer.Key.GetComponent<L4A235_drag>();
    ...
    public void ResetDragDrop()
    {
        if (first)
        {
            ...
            foreach (var drop in _drops)
            {
                ...
                drop._drag = null;
                drop.GetComponent<Image>().color = new Color32(255,255,255,255);
            }
