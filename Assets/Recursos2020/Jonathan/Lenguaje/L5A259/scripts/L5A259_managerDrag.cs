...
public class L5A259_managerDrag : MonoBehaviour
{
    public L5A259_isla isla;
    public bool first;
    ...
    [Header("Item List")]
    public List<L5A259_drag> _drags;
    public List<L5A259_drop> _drops;

    public List<L5A259_grpoDrag> _groups;
    ...
    void RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<L5A259_drag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
            if (drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<L5A259_drop>())));


        foreach (var drop in _drops)
            if (drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<L5A259_drag>()._DropRight.Contains(drop)));

        foreach (var answer in answers)
        {
            var BhDrop = answer.Key.GetComponent<L5A259_drop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<L5A259_grpoDrag>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<L5A259_grpoDrag>()._currentWrong++;
                }
            }
            //print(answer.Key.GetComponent<L5A259_drop>()._group+""+answer.Key + answer);
        }
    ...
    public void ImmediatelyValidation(L5A259_drag drag, L5A259_drop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<L5A259_drag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<L5A259_drop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
