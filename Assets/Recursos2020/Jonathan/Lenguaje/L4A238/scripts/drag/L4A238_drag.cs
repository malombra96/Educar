...
public class L4A238_drag : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IDropHandler, IPointerClickHandler
{
    L4A238_managerDrag _L4A238_managerDrag;
    ...
    [Tooltip("Arrastre aquí los drop correctos")] [Header("Correct Drop")] public List<L4A238_drop> _DropRight;
    ...
    private void Awake()
    {
        ...
        _L4A238_managerDrag = FindObjectOfType<L4A238_managerDrag>();
        _L4A238_managerDrag._drags.Add(this);
    }

    public void OnBeginDrag(PointerEventData eventData)
    {
        _L4A238_managerDrag._controlAudio.PlayAudio(0);
    ...
    {
        if (!_pivotDraging)
            _rectTransform.anchoredPosition += eventData.delta / _L4A238_managerDrag.canvas.scaleFactor;
        else
        {
            float Y = Display.main.systemHeight / 2;
            float X = Display.main.systemWidth / 2;

            Vector2 newEventData = new Vector2((eventData.position.x - X), (eventData.position.y - Y)) / _L4A238_managerDrag.canvas.scaleFactor;
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        if (!swap)
        {
            ...
            else
            {
                ...
                if (ItsInfinite && _L4A238_managerDrag._OperatingMethod == L4A238_managerDrag.OperatingMethod.Create)
                    CreateDrag();
            }
        }
        else
        {
            ...
            UpdateCurrentPosition();
            SetSpriteState(false);
            GetComponent<L4A238_drag>().swap = false;
        }
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        if (other.GetComponent<L4A238_drag>() && _L4A238_managerDrag._OperatingMethod != L4A238_managerDrag.OperatingMethod.Create)
        {
    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        ...
        if (_drop)
        {
            _drop.GetComponent<L4A238_drop>()._drag = null;
            _drop = null;
        }

        if (_L4A238_managerDrag._OperatingMethod == L4A238_managerDrag.OperatingMethod.Create)
        {
            if (_L4A238_managerDrag._drags.Contains(this) && !ItsInfinite)
            {
                _L4A238_managerDrag._drags.Remove(this);
        ...
        UpdateCurrentPosition();
        StartCoroutine(_L4A238_managerDrag.StateBtnValidar());
    ...
    {
        other.GetComponent<L4A238_drag>().swap = true; // Estado de intercambio 
        Vector2 a = other.GetComponent<L4A238_drag>()._currentPos;
    ...
    public void SwapDefaultPosition(GameObject other)
    {
        ...
        Vector2 x = other.GetComponent<L4A238_drag>()._defaultPos;
        other.GetComponent<L4A238_drag>()._defaultPos = _defaultPos;
        _defaultPos = x;
    }
    ...
    void SwapDrag(GameObject other)
    {
        ...
        GameObject m = other.GetComponent<L4A238_drag>()._drop;
        GameObject n = _drop;

        if (m)
            other.GetComponent<L4A238_drag>()._drop.GetComponent<L4A238_drop>()._drag = other;

        if (n)
            _drop.GetComponent<L4A238_drop>()._drag = gameObject;
    ...
    {
        GameObject x = other.GetComponent<L4A238_drag>()._drop;
        other.GetComponent<L4A238_drag>()._drop = _drop;
        _drop = x;
    ...
    {
        if (_L4A238_managerDrag._OperatingMethod == L4A238_managerDrag.OperatingMethod.Match)
        {
            int x = other.GetComponent<L4A238_drag>()._row;
            other.GetComponent<L4A238_drag>()._row = this._row;
            this._row = x;
        }
    ...
    {
        bool x = other.GetComponent<L4A238_drag>().inDrop;
        other.GetComponent<L4A238_drag>().inDrop = this.inDrop;
        this.inDrop = x;
    }
    ...
    void CreateDrag()
    {
        GameObject aux = Instantiate(gameObject, _L4A238_managerDrag.transform);
        aux.name = name;
    ...
    void InicializatNewDrag(GameObject aux)
    {
        ...
        aux.GetComponent<L4A238_drag>()._defaultPos = aux.GetComponent<RectTransform>().anchoredPosition;
        aux.GetComponent<L4A238_drag>().inDrop = false;
        aux.GetComponent<L4A238_drag>().swap = false;
        aux.GetComponent<L4A238_drag>()._drop = null;
        aux.GetComponent<L4A238_drag>().ItsInfinite = true;
