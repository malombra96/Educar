...
public class L4A238_managerDrag : MonoBehaviour
{
    ...
    [Header("Item List")]
    public List<L4A238_drag> _drags;
    public List<L4A238_drop> _drops;

    public List<L4A238_groupDrag> _groups;
    ...
    void RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<L4A238_drag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
            if (drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<L4A238_drop>())));
        ...
        {
            var BhDrop = answer.Key.GetComponent<L4A238_drop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<L4A238_groupDrag>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<L4A238_groupDrag>()._currentWrong++;
                }
            }
            //print(answer.Key.GetComponent<L4A238_drop>()._group+""+answer.Key + answer);
        }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<L4A238_drag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<L4A238_drop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    public void SetPuntajeGroups()
    {
        ...
        if (input) {
            ...
            string x = field.text.ToLower();
            if (rights == _groups.Count)
            {
                
                if (x.Contains(palabra))
                {
                    field.textComponent.GetComponent<Text>().color = verde;
                    _controlAudio.PlayAudio(1);
                    _controlPuntaje.IncreaseScore(rights);
                }
                else {
                    field.textComponent.GetComponent<Text>().color = rojo;
                    _controlAudio.PlayAudio(2);
                }
    ...
    public void ResetDragDrop()
    {
        if (first)
        {
            iniciar.transform.SetAsLastSibling();
            if (input) {
                field.interactable = true;
                field.text = "";
                field.textComponent.GetComponent<Text>().color = Color.black;
            }
