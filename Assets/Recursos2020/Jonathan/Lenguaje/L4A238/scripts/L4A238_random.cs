...
public class L4A238_random : MonoBehaviour
{
    ...
    private void Start()
    {
        for (int j = 0; j < _TargetsList.Count; j++)
        {
            _TargetsList[j].GetComponent<L4A238_toggleDraw>()._point = _TargetsList[j].GetComponent<RectTransform>().position;
            _TargetsList[j].GetComponent<L4A238_toggleDraw>()._point.z = -9.6f;
        }
    }
}
