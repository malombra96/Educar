...
public class L10A189_manager : MonoBehaviour
{
    public bool first;
    ...
    [Header("Item List")]
    public List<L10A189_drag> _drags;
    public List<L10A189_drop> _drops;

    public List<L10A189_grop> _groups;
    ...
    void RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<L10A189_drag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
            if (drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<L10A189_drop>())));


        foreach (var drop in _drops)
            if (drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<L10A189_drag>()._DropRight.Contains(drop)));

        foreach (var answer in answers)
        {
            var BhDrop = answer.Key.GetComponent<L10A189_drop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<L10A189_grop>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<L10A189_grop>()._currentWrong++;
                }
            }
            //print(answer.Key.GetComponent<L10A189_drop>()._group+""+answer.Key + answer);
        }
    ...
    public void ImmediatelyValidation(L10A189_drag drag, L10A189_drop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<L10A189_drag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<L10A189_drop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    public void SetPuntaje()
    {
        ...
        foreach (var answer in answers)
        {
            L10A189_drag temp = answer.Key.GetComponent<L10A189_drag>();
