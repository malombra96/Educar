...
public class L5A256_managerDrag : MonoBehaviour
{
    public L5A256_general general;
    public bool first;
    ...
    [Header("Item List")]
    public List<L5A256_drag> _drags;
    public List<L5A256_drop> _drops;

    public List<L5A256_group> _groups;
    ...
    void RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<L5A256_drag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
            if (drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<L5A256_drop>())));


        foreach (var drop in _drops)
            if (drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<L5A256_drag>()._DropRight.Contains(drop)));

        foreach (var answer in answers)
        {
            var BhDrop = answer.Key.GetComponent<L5A256_drop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<L5A256_group>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<L5A256_group>()._currentWrong++;
                }
            }
            //print(answer.Key.GetComponent<L5A256_drop>()._group+""+answer.Key + answer);
        }
    ...
    public void ImmediatelyValidation(L5A256_drag drag, L5A256_drop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<L5A256_drag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<L5A256_drop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    public void SetPuntaje()
    {
        ...
        foreach (var answer in answers)
        {
            L5A256_drag temp = answer.Key.GetComponent<L5A256_drag>();
        ...
        else
        {
            if (_customValidar == 0) {
                if (rights == _drags.Count) {
                    ...
                    general.pregunta(true);
                } 
                else {
                    _controlAudio.PlayAudio(2);
                    general.pregunta(false);
                }
            }
    ...
    public void ResetDragDrop()
    {
        if (first)
        {
            ...
            foreach (var drag in _drags)
            {
                ...
                drag.enabled = true;
                drag._drop = null;
                drag.GetComponent<RectTransform>().sizeDelta = drag.tamIni;
