...
public class MA832_managerInput : MonoBehaviour
{
    public MA832_managerCables managerCables;
    ...
    public List<MA832_group> _groupInputField;
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<MA832_input>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<MA832_input>()._isRight = false;
                inputField.GetComponent<MA832_input>()._isEnabled = true;
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == MA832_group.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    InputField f = _groupInputField[i]._inputFields[j];
                    f.interactable = false;
                    f.GetComponent<MA832_input>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<MA832_input>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<MA832_input>()._isRight;
        ...
        if (groupTemp == _groupInputField.Count)
        {
            ...
            cable.GetComponent<BehaviourSprite>()._disabled = cable.GetComponent<BehaviourSprite>()._right;
            StartCoroutine(x(true));
        }
        else {
            _controlAudio.PlayAudio(2);
            cable.GetComponent<Image>().sprite = cable.GetComponent<BehaviourSprite>()._wrong;
            cable.GetComponent<BehaviourSprite>()._disabled = cable.GetComponent<BehaviourSprite>()._wrong;
    ...
    IEnumerator x(bool value) {
        ...
        if (value)
        {
            ...
            cable.GetComponent<Toggle>().interactable = false;
            cable.GetComponent<Toggle>().transition = Selectable.Transition.None;
            _controlNavegacion.GoToLayout(2);
        }
        else {
            
            managerCables.ActivateBombs(managerCables.boom);
            managerCables.boom++;
    ...
    {
        InitializeState();
        managerCables.count = 0;
        cable.GetComponent<Toggle>().interactable = true;
        cable.GetComponent<Toggle>().transition = Selectable.Transition.SpriteSwap;
        cable.GetComponent<Image>().sprite = cable.GetComponent<BehaviourSprite>()._default ;
        calificado = false;
        managerCables.boom = 0;
        managerCables.explosion.SetActive(false);
        managerCables.bomba.SetActive(true);
        managerCables.marcador.SetActive(true);
        managerCables.cables.SetActive(true);

        for (int i = 0; i < managerCables.bombs.transform.childCount;i++) {
            managerCables.bombs.transform.GetChild(i).gameObject.GetComponent<Image>().sprite = managerCables.bombs.transform.GetChild(0).gameObject.GetComponent<BehaviourSprite>()._default;
        }
