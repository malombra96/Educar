...
public class M7A124_managerDrag : MonoBehaviour
{

    public bool first;
    public ControlPuntaje _controlPuntaje;
    public ControlNavegacion _controlNavegacion;
    [HideInInspector] public ControlAudio _controlAudio;
    [HideInInspector] public Canvas canvas;

    [Header("Item List")]
    public List<M7A124_drag> _drags;
    public List<M7A124_drop> _drops;

    public List<M7A124_group> _groups;
    ...
    [Header("Activacion Manual de Boton Validar")] [Range(0, 20)] public int _customValidar;
    ...
    void RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<M7A124_drag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
            if (drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<M7A124_drop>())));


        foreach (var drop in _drops)
            if (drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<M7A124_drag>()._DropRight.Contains(drop)));

        foreach (var answer in answers)
        {
            var BhDrop = answer.Key.GetComponent<M7A124_drop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<M7A124_group>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<M7A124_group>()._currentWrong++;
                }
            }
            //print(answer.Key.GetComponent<M7A124_drop>()._group+""+answer.Key + answer);
        }
    ...
    public void ImmediatelyValidation(M7A124_drag drag, M7A124_drop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M7A124_drag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M7A124_drop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    public void SetPuntaje()
    {
        ...
        foreach (var answer in answers)
        {
            M7A124_drag temp = answer.Key.GetComponent<M7A124_drag>();
