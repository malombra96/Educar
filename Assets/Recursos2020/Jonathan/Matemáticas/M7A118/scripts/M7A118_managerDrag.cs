...
public class M7A118_managerDrag : MonoBehaviour
{
    public M7A118_general general;
    public int valor;
    public List<GameObject> siguienteFIcha;
    public bool first,ultimo;
    ...
    [Header("Item List")]
    public List<M7A118_drag> _drags;
    public List<M7A118_drop> _drops;

    public List<M7A118_grupoDrag> _groups;
    ...
    void RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<M7A118_drag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
            if (drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<M7A118_drop>())));


        foreach (var drop in _drops)
            if (drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<M7A118_drag>()._DropRight.Contains(drop)));

        foreach (var answer in answers)
        {
            var BhDrop = answer.Key.GetComponent<M7A118_drop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<M7A118_grupoDrag>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<M7A118_grupoDrag>()._currentWrong++;
                }
            }
            //print(answer.Key.GetComponent<M7A118_drop>()._group+""+answer.Key + answer);
        }
    ...
    public void ImmediatelyValidation(M7A118_drag drag, M7A118_drop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M7A118_drag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M7A118_drop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    public void SetPuntaje()
    {
        ...
        foreach (var answer in answers)
        {
            M7A118_drag temp = answer.Key.GetComponent<M7A118_drag>();
        ...
        else
        {
            if (_customValidar == 0) {
                ...
                {
                    general.Calificar(valor,true);
                    _controlAudio.PlayAudio(1);
                }
                else {
                    general.Calificar(valor, false);
