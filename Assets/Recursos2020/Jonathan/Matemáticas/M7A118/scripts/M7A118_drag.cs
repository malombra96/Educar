...
public class M7A118_drag : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IDropHandler, IPointerClickHandler
{
    M7A118_managerDrag _M7A118_managerDrag;
    ...
    [Tooltip("Arrastre aquí los drop correctos")] [Header("Correct Drop")] public List<M7A118_drop> _DropRight;
    ...
    private void Awake()
    {
        ...
        _M7A118_managerDrag._drags.Add(this);
    }

    public void OnBeginDrag(PointerEventData eventData)
    {
        _M7A118_managerDrag._controlAudio.PlayAudio(0);
    ...
    {
        if (!_pivotDraging)
            _rectTransform.anchoredPosition += eventData.delta / _M7A118_managerDrag.canvas.scaleFactor;
        else
        {
            float Y = Display.main.systemHeight / 2;
            float X = Display.main.systemWidth / 2;

            Vector2 newEventData = new Vector2((eventData.position.x - X), (eventData.position.y - Y)) / _M7A118_managerDrag.canvas.scaleFactor;
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        if (!swap)
        {
            ...
            else
            {
                ...
                if (ItsInfinite && _M7A118_managerDrag._OperatingMethod == M7A118_managerDrag.OperatingMethod.Create)
                    CreateDrag();
            }
        }
        else
        {
            ...
            UpdateCurrentPosition();
            SetSpriteState(false);
            GetComponent<M7A118_drag>().swap = false;
        }
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        if (other.GetComponent<M7A118_drag>() && _M7A118_managerDrag._OperatingMethod != M7A118_managerDrag.OperatingMethod.Create)
        {
    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        ...
        if (_drop)
        {
            _drop.GetComponent<M7A118_drop>()._drag = null;
            _drop = null;
        }

        if (_M7A118_managerDrag._OperatingMethod == M7A118_managerDrag.OperatingMethod.Create)
        {
            if (_M7A118_managerDrag._drags.Contains(this) && !ItsInfinite)
            {
                _M7A118_managerDrag._drags.Remove(this);
    ...
    {
        other.GetComponent<M7A118_drag>().swap = true; // Estado de intercambio 
        Vector2 a = other.GetComponent<M7A118_drag>()._currentPos;
    ...
    public void SwapDefaultPosition(GameObject other)
    {
        ...
        Vector2 x = other.GetComponent<M7A118_drag>()._defaultPos;
        other.GetComponent<M7A118_drag>()._defaultPos = _defaultPos;
        _defaultPos = x;
    }
    ...
    void SwapDrag(GameObject other)
    {
        ...
        GameObject m = other.GetComponent<M7A118_drag>()._drop;
        GameObject n = _drop;

        if (m)
            other.GetComponent<M7A118_drag>()._drop.GetComponent<M7A118_drop>()._drag = other;

        if (n)
            _drop.GetComponent<M7A118_drop>()._drag = gameObject;
    ...
    {
        GameObject x = other.GetComponent<M7A118_drag>()._drop;
        other.GetComponent<M7A118_drag>()._drop = _drop;
        _drop = x;
    ...
    {
        bool x = other.GetComponent<M7A118_drag>().inDrop;
        other.GetComponent<M7A118_drag>().inDrop = this.inDrop;
        this.inDrop = x;
    }
    ...
    void CreateDrag()
    {
        GameObject aux = Instantiate(gameObject, _M7A118_managerDrag.transform);
        aux.name = name;
    ...
    void InicializatNewDrag(GameObject aux)
    {
        ...
        aux.GetComponent<M7A118_drag>()._defaultPos = aux.GetComponent<RectTransform>().anchoredPosition;
        aux.GetComponent<M7A118_drag>().inDrop = false;
        aux.GetComponent<M7A118_drag>().swap = false;
        aux.GetComponent<M7A118_drag>()._drop = null;
        aux.GetComponent<M7A118_drag>().ItsInfinite = true;
