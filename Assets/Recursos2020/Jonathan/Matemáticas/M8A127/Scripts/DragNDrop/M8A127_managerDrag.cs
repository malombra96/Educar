...
public class M8A127_managerDrag : MonoBehaviour
{
    public M8A127_managerGeneral general;
    public bool first;
    ...
    [Header("Item List")]
    public List<M8A127_drag> _drags;
    public List<M8A127_Drop> _drops;

    public List<M8A127_groupDrag> _groups;
    ...
    void RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<M8A127_drag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
            if (drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<M8A127_Drop>())));


        foreach (var drop in _drops)
            if (drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<M8A127_drag>()._DropRight.Contains(drop)));

        foreach (var answer in answers)
        {
            var BhDrop = answer.Key.GetComponent<M8A127_Drop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<M8A127_groupDrag>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<M8A127_groupDrag>()._currentWrong++;
                }
            }
            //print(answer.Key.GetComponent<M8A127_Drop>()._group+""+answer.Key + answer);
        }
    ...
    public void ImmediatelyValidation(M8A127_drag drag, M8A127_Drop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M8A127_drag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M8A127_Drop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    public void SetPuntaje()
    {
        ...
        foreach (var answer in answers)
        {
            M8A127_drag temp = answer.Key.GetComponent<M8A127_drag>();
        ...
        else
        {
            if (_customValidar == 0)
                ...
                if (rights == _customValidar) {
                    ...
                    StartCoroutine(general.NextExcersise(true));
                }
                else {
                    _controlAudio.PlayAudio(2);
                    StartCoroutine(general.NextExcersise(false));
                }
            }
