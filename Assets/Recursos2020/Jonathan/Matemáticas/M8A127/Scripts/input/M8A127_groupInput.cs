...
public class M8A127_groupInput : MonoBehaviour
{
    public M8A127_managerInput _M8A127_managerInput;
        ...
        for (int i = 0; i < _inputFields.Count; i++)
        {
            for (int j = 0; j < temporary.Count; j++)
            {
                if (temporary.Contains(_inputFields[i].text))
                {
                    ...
                    {
                        _inputFields[i].GetComponent<M8A127_input>()._isRight = true;
                ...
                {
                    _inputFields[i].GetComponent<M8A127_input>()._isRight = false;
                    break;
                }
