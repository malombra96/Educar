...
public class M5A115_grupoInput : MonoBehaviour
{
    public M5A115_managerInput _M5A115_managerInput;
        ...
        for (int i = 0; i < _inputFields.Count; i++)
        {
            for (int j = 0; j < temporary.Count; j++)
            {
                if (temporary.Contains(_inputFields[i].text))
                {
                    ...
                    {
                        _inputFields[i].GetComponent<M5A115_input>()._isRight = true;
                ...
                {
                    _inputFields[i].GetComponent<M5A115_input>()._isRight = false;
                    break;
                }
