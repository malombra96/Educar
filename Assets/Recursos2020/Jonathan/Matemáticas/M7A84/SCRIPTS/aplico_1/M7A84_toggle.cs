...
public class M7A84_toggle : MonoBehaviour
{
    ControlAudio _controlAudio;
    Toggle _toggle;
    [Header("is Right?")] public bool isRight;
    M7A84_grupo _behaviourToggleGroup;
    M7A84_managerSeleccionar _managerToggle;

    void Start()
    {
        _controlAudio = FindObjectOfType<ControlAudio>();
        _toggle = GetComponent<Toggle>();
        _behaviourToggleGroup = transform.parent.GetComponent<M7A84_grupo>();
        _managerToggle = transform.parent.parent.GetComponent<M7A84_managerSeleccionar>();
        _toggle.onValueChanged.AddListener(delegate { SetStateToggle(); });

        if (_managerToggle._NeedSymbol == M7A84_managerSeleccionar.NeedSymbol.withSymbol)
            ...
            case M7A84_managerSeleccionar.TypeSelection.onexgroup:
                ...
                if (_toggle.isOn)
                {
                    ...
                    switch (_managerToggle._typeVal)
                    {
                        ...
                        case M7A84_managerSeleccionar.TypeValidation.inmediata:
