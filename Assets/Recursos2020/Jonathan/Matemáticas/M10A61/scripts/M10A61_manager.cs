...
public class M10A61_manager : MonoBehaviour
{
    ...
    [Header("Item List")]
    public List<M10A61_drag> _drags;
    public List<M10A61_Drop> _drops;

    public List<M10A61_groupDrag> _groups;
    ...
    void RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<M10A61_drag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
            if (drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<M10A61_Drop>())));


        foreach (var drop in _drops)
            if (drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<M10A61_drag>()._DropRight.Contains(drop)));

        foreach (var answer in answers)
        {
            var BhDrop = answer.Key.GetComponent<M10A61_Drop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<M10A61_groupDrag>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<M10A61_groupDrag>()._currentWrong++;
                }
            }
    ...
    public void ImmediatelyValidation(M10A61_drag drag, M10A61_Drop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M10A61_drag>())
    ...
    public void SetPuntaje()
    {
        ...
        foreach (var answer in answers)
        {
            M10A61_drag temp = answer.Key.GetComponent<M10A61_drag>();
