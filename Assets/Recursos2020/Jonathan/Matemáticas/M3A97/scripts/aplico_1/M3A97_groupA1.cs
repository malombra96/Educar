...
public class M3A97_groupA1 : MonoBehaviour
{
    public M3A97_managerInputA1 _managerInputField;
        ...
        for (int i = 0; i < _inputFields.Count; i++)
        {
            for (int j = 0; j < temporary.Count; j++)
            {
                if (temporary.Contains(_inputFields[i].text))
                {
                    ...
                    {
                        _inputFields[i].GetComponent<M3A97_inputA1>()._isRight = true;
                ...
                {
                    _inputFields[i].GetComponent<M3A97_inputA1>()._isRight = false;
                    break;
                }
