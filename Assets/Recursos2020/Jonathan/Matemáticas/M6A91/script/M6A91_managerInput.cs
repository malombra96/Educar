...
public class M6A91_managerInput : MonoBehaviour
{
    public GameObject explosionR, explosionW,barco;
    ...
    public List<M6A91_groupInput> _groupInputField;
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M6A91_input>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    public void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M6A91_input>()._isRight = false;
                inputField.GetComponent<M6A91_input>()._isEnabled = true;
    ...
    void SetTextAnswer(Text text, bool state) {
        if (state)
        {
            ...
            //explosionR.GetComponent<Animator>().SetBool("x",true);
        }
        else {
            text.color = colorTextoIncorrecto;
            explosionW.SetActive(true);
            //explosionW.GetComponent<Animator>().SetBool("x", true);
        }
    ...
    public void resetAll()
    {
        ...
        explosionR.GetComponent<Animator>().SetBool("x", false);
        explosionW.GetComponent<Animator>().SetBool("x", false);
        explosionR.SetActive(false);
        explosionW.SetActive(false);
