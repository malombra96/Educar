...
public class M6A91_toggle : MonoBehaviour
{
    ControlAudio _controlAudio;
    Toggle _toggle;
    [Header("is Right?")] public bool isRight;
    M6A91_toggleGroup _behaviourToggleGroup;
    M6A91_managerSeleccion _managerToggle;

    void Start()
    {
        _controlAudio = FindObjectOfType<ControlAudio>();
        _toggle = GetComponent<Toggle>();
        _behaviourToggleGroup = transform.parent.GetComponent<M6A91_toggleGroup>();
        _managerToggle = transform.parent.parent.GetComponent<M6A91_managerSeleccion>();
        _toggle.onValueChanged.AddListener(delegate { SetStateToggle(); });

        if (_managerToggle._NeedSymbol == M6A91_managerSeleccion.NeedSymbol.withSymbol)
    ...
    void SetStateToggle()
    {
        ...
        {
            case M6A91_managerSeleccion.TypeSelection.onexgroup:
                ...
                if (_toggle.isOn)
                {
                    ...
                    switch (_managerToggle._typeVal)
                    {
                        ...
                        case M6A91_managerSeleccion.TypeValidation.inmediata:
            ...
            case M6A91_managerSeleccion.TypeSelection.variousxgroup:
