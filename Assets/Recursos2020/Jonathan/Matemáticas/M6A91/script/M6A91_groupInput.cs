...
public class M6A91_groupInput : MonoBehaviour
{
    public M6A91_managerInput _managerInputField;
        ...
        for (int i = 0; i < _inputFields.Count; i++)
        {
            for (int j = 0; j < temporary.Count; j++)
            {
                if (temporary.Contains(_inputFields[i].text))
                {
                    ...
                    {
                        _inputFields[i].GetComponent<M6A91_input>()._isRight = true;
                ...
                {
                    _inputFields[i].GetComponent<M6A91_input>()._isRight = false;
                    break;
                }
