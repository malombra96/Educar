...
public class M9A81_seleccionarA2 : MonoBehaviour
{
    ControlAudio _controlAudio;
    Toggle _toggle;
    [Header("is Right?")] public bool isRight;
    M9A81_grupoA2 _behaviourToggleGroup;
    M9A81_managerSeleccionarA2 _managerToggle;

    void Start()
    {
        _controlAudio = FindObjectOfType<ControlAudio>();
        _toggle = GetComponent<Toggle>();
        _behaviourToggleGroup = transform.parent.GetComponent<M9A81_grupoA2>();
        _managerToggle = transform.parent.parent.GetComponent<M9A81_managerSeleccionarA2>();
        _toggle.onValueChanged.AddListener(delegate { SetStateToggle(); });

        if (_managerToggle._NeedSymbol == M9A81_managerSeleccionarA2.NeedSymbol.withSymbol)
            ...
            case M9A81_managerSeleccionarA2.TypeSelection.onexgroup:
                ...
                if (_toggle.isOn)
                {
                    ...
                    switch (_managerToggle._typeVal)
                    {
                        ...
                        case M9A81_managerSeleccionarA2.TypeValidation.inmediata:
