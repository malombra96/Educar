...
public class M9A81_toggle : MonoBehaviour
{
    ControlAudio _controlAudio;
    Toggle _toggle;
    [Header("is Right?")] public bool isRight;
    M9A81_grupo _behaviourToggleGroup;
    M9A81_managerSeleccionar _managerToggle;

    void Start()
    {
        _controlAudio = FindObjectOfType<ControlAudio>();
        _toggle = GetComponent<Toggle>();
        _behaviourToggleGroup = transform.parent.GetComponent<M9A81_grupo>();
        _managerToggle = transform.parent.parent.GetComponent<M9A81_managerSeleccionar>();
        _toggle.onValueChanged.AddListener(delegate { SetStateToggle(); });

        if (_managerToggle._NeedSymbol == M9A81_managerSeleccionar.NeedSymbol.withSymbol)
    ...
    void SetStateToggle()
    {
        ...
        {
            case M9A81_managerSeleccionar.TypeSelection.onexgroup:
                ...
                if (_toggle.isOn)
                {
                    ...
                    switch (_managerToggle._typeVal)
                    {
                        ...
                        case M9A81_managerSeleccionar.TypeValidation.inmediata:
            ...
            case M9A81_managerSeleccionar.TypeSelection.variousxgroup:
