...
public class M7A119_drag : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IDropHandler, IPointerClickHandler
{
    M7A119_dragManager _M7A119_dragManager;
    ...
    [Tooltip("Arrastre aquí los drop correctos")] [Header("Correct Drop")] public List<M7A119_drop> _DropRight;
    ...
    private void Awake()
    {
        ...
        _M7A119_dragManager._drags.Add(this);
    }

    public void OnBeginDrag(PointerEventData eventData)
    {
        _M7A119_dragManager._controlAudio.PlayAudio(0);
    ...
    {
        if (!_pivotDraging)
            _rectTransform.anchoredPosition += eventData.delta / _M7A119_dragManager.canvas.scaleFactor;
        else
        {
            float Y = Display.main.systemHeight / 2;
            float X = Display.main.systemWidth / 2;

            Vector2 newEventData = new Vector2((eventData.position.x - X), (eventData.position.y - Y)) / _M7A119_dragManager.canvas.scaleFactor;
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        if (!swap)
        {
            ...
            else
            {
                ...
                if (ItsInfinite && _M7A119_dragManager._OperatingMethod == M7A119_dragManager.OperatingMethod.Create)
                    CreateDrag();
            }
        }
        else
        {
            ...
            UpdateCurrentPosition();
            SetSpriteState(false);
            GetComponent<BehaviourDrag>().swap = false;
        }
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        if (other.GetComponent<BehaviourDrag>() && _M7A119_dragManager._OperatingMethod != M7A119_dragManager.OperatingMethod.Create)
        {
    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        ...
        if (_drop)
        {
            _drop.GetComponent<M7A119_drop>()._drag = null;
            _drop = null;
        }

        if (_M7A119_dragManager._OperatingMethod == M7A119_dragManager.OperatingMethod.Create)
        {
            if (_M7A119_dragManager._drags.Contains(this) && !ItsInfinite)
            {
                _M7A119_dragManager._drags.Remove(this);
    ...
    {
        other.GetComponent<BehaviourDrag>().swap = true; // Estado de intercambio 
    ...
    public void SwapDefaultPosition(GameObject other)
    {
        ...
        Vector2 x = other.GetComponent<BehaviourDrag>()._defaultPos;
        other.GetComponent<BehaviourDrag>()._defaultPos = _defaultPos;
        _defaultPos = x;
    }
    ...
    void SwapDrag(GameObject other)
    {
        ...
        GameObject m = other.GetComponent<BehaviourDrag>()._drop;
        GameObject n = _drop;

        if (m)
            other.GetComponent<BehaviourDrag>()._drop.GetComponent<M7A119_drop>()._drag = other;

        if (n)
            _drop.GetComponent<M7A119_drop>()._drag = gameObject;
    ...
    {
        GameObject x = other.GetComponent<BehaviourDrag>()._drop;
        other.GetComponent<BehaviourDrag>()._drop = _drop;
        _drop = x;
    ...
    {
        bool x = other.GetComponent<BehaviourDrag>().inDrop;
        other.GetComponent<BehaviourDrag>().inDrop = this.inDrop;
        this.inDrop = x;
    }
    ...
    void CreateDrag()
    {
        GameObject aux = Instantiate(gameObject, _M7A119_dragManager.transform);
        aux.name = name;
    ...
    void InicializatNewDrag(GameObject aux)
    {
        ...
        aux.GetComponent<BehaviourDrag>()._defaultPos = aux.GetComponent<RectTransform>().anchoredPosition;
        aux.GetComponent<BehaviourDrag>().inDrop = false;
        aux.GetComponent<BehaviourDrag>().swap = false;
        aux.GetComponent<BehaviourDrag>()._drop = null;
        aux.GetComponent<BehaviourDrag>().ItsInfinite = true;
