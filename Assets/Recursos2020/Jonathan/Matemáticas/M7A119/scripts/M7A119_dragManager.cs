...
public class M7A119_dragManager : MonoBehaviour
{
    public M7A119_managerGeneral general;
    public bool first;
    ...
    [Header("Item List")]
    public List<M7A119_drag> _drags;
    public List<M7A119_drop> _drops;

    public List<M7A119_groupDrop> _groups;
    ...
    void RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<M7A119_drag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
            if (drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<M7A119_drop>())));


        foreach (var drop in _drops)
            if (drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<M7A119_drag>()._DropRight.Contains(drop)));

        foreach (var answer in answers)
        {
            var BhDrop = answer.Key.GetComponent<M7A119_drop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<M7A119_groupDrop>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<M7A119_groupDrop>()._currentWrong++;
                }
            }
            //print(answer.Key.GetComponent<M7A119_drop>()._group+""+answer.Key + answer);
        }
    ...
    public void ImmediatelyValidation(M7A119_drag drag, M7A119_drop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M7A119_drag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M7A119_drop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
