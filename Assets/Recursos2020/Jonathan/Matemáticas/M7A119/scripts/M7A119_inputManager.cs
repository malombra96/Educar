...
public class M7A119_inputManager : MonoBehaviour
{
    ...
    public List<M7A119_groupInpu> _groupInputField;
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M7A119_input>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    public void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M7A119_input>()._isRight = false;
                inputField.GetComponent<M7A119_input>()._isEnabled = true;
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == M7A119_groupInpu.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<M7A119_input>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<M7A119_input>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<M7A119_input>()._isRight;
        ...
        if (groupTemp == _groupInputField.Count) {
            general.NextExcersiseInput(true);
            _controlAudio.PlayAudio(1);
        }
        else {
            general.NextExcersiseInput(false);
