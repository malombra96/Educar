...
public class M8A39_ManagerInputField : MonoBehaviour
{
    ...
    [Header("Tipo de puntuacion")] public TipoPuntuacion _TipoPuntuacion;
     ...
     public List<M8A39_BehaviourGroupInputField> _groupInputField;
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M8A39_input>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M8A39_input>()._isRight = false;
                inputField.GetComponent<M8A39_input>()._isEnabled = true;
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == M8A39_BehaviourGroupInputField.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<M8A39_input>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<M8A39_input>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<M8A39_input>()._isRight;
        ...
        if (groupTemp == _groupInputField.Count)
        {
            ...
            manager.AddGoals(true,gameObject);
            manager.AddCantidad(10);
            PlayerPrefs.SetFloat("actividad2", groupTemp);
        }
        else {
            _controlAudio.PlayAudio(2);
            manager.AddGoals(false, gameObject);
            manager.RestLife();
        }
