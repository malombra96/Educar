...
public class M3A100_toggle : MonoBehaviour
{
    ControlAudio _controlAudio;
    Toggle _toggle;
    [Header("is Right?")] public bool isRight;
    M3A100_groupToggle _behaviourToggleGroup;
    M3A100_managerSeleccionar _managerToggle;

    void Start()
    {
        _controlAudio = FindObjectOfType<ControlAudio>();
        _toggle = GetComponent<Toggle>();
        _behaviourToggleGroup = transform.parent.GetComponent<M3A100_groupToggle>();
        _managerToggle = transform.parent.parent.GetComponent<M3A100_managerSeleccionar>();
        _toggle.onValueChanged.AddListener(delegate { SetStateToggle(); });

        if (_managerToggle._NeedSymbol == M3A100_managerSeleccionar.NeedSymbol.withSymbol)
            ...
            case M3A100_managerSeleccionar.TypeSelection.onexgroup:
                ...
                if (_toggle.isOn)
                {
                    ...
                    switch (_managerToggle._typeVal)
                    {
                        ...
                        case M3A100_managerSeleccionar.TypeValidation.inmediata:
