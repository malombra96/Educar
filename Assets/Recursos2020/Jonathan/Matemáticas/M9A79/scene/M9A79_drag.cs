...
public class M9A79_drag : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler /*IDropHandler*/, IPointerClickHandler
{
    M9A79_managerDragDrop _M9A79_managerDragDrop;
    ...
    [Tooltip("Arrastre aquí los drop correctos")] [Header("Correct Drop")] public List<M9A79_drop> _DropRight;
    ...
    private void Awake()
    {
        ...
        _M9A79_managerDragDrop._drags.Add(this);
    }

    public void OnBeginDrag(PointerEventData eventData)
    {
        _M9A79_managerDragDrop._controlAudio.PlayAudio(0);
    ...
    {
        if (!_pivotDraging)
            _rectTransform.anchoredPosition += eventData.delta / _M9A79_managerDragDrop.canvas.scaleFactor;
        else
        {
            float Y = Display.main.systemHeight / 2;
            float X = Display.main.systemWidth / 2;

            Vector2 newEventData = new Vector2((eventData.position.x - X), (eventData.position.y - Y)) / _M9A79_managerDragDrop.canvas.scaleFactor;
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        if (!swap)
        {
            ...
            else
            {
                ...
                if (ItsInfinite && _M9A79_managerDragDrop._OperatingMethod == M9A79_managerDragDrop.OperatingMethod.Create)
                    CreateDrag();
            }
        }
        else
        {
            ...
            UpdateCurrentPosition();
            SetSpriteState(false);
            GetComponent<M9A79_drag>().swap = false;
        }
    ...
    //    if (other.GetComponent<M9A79_drag>() && _M9A79_managerDragDrop._OperatingMethod != M9A79_managerDragDrop.OperatingMethod.Create)
    //    {
    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        ...
        if (_drop)
        {
            _drop.GetComponent<M9A79_drop>()._drag = null;
            _drop = null;
        }

        if (_M9A79_managerDragDrop._OperatingMethod == M9A79_managerDragDrop.OperatingMethod.Create)
        {
            if (_M9A79_managerDragDrop._drags.Contains(this) && !ItsInfinite)
            {
                _M9A79_managerDragDrop._drags.Remove(this);
    ...
    //{
    //    other.GetComponent<M9A79_drag>().swap = true; // Estado de intercambio 
    //    Vector2 a = other.GetComponent<M9A79_drag>()._currentPos;
    ...
    public void SwapDefaultPosition(GameObject other)
    {
        ...
        Vector2 x = other.GetComponent<M9A79_drag>()._defaultPos;
        other.GetComponent<M9A79_drag>()._defaultPos = _defaultPos;
        _defaultPos = x;
    }
    ...
    //    GameObject m = other.GetComponent<M9A79_drag>()._drop;
    //    GameObject n = _drop;

    //    if (m)
    //        other.GetComponent<M9A79_drag>()._drop.GetComponent<M9A79_drop>()._drag = other;

    //    if (n)
    //        _drop.GetComponent<M9A79_drop>()._drag = gameObject;
    ...
    //{
    //    GameObject x = other.GetComponent<M9A79_drag>()._drop;
    //    other.GetComponent<M9A79_drag>()._drop = _drop;
    //    _drop = x;
    ...
    //{
    //    bool x = other.GetComponent<M9A79_drag>().inDrop;
    //    other.GetComponent<M9A79_drag>().inDrop = this.inDrop;
    //    this.inDrop = x;
    //}
    ...
    void CreateDrag()
    {
        GameObject aux = Instantiate(gameObject, _M9A79_managerDragDrop.transform);
        aux.name = name;
    ...
    void InicializatNewDrag(GameObject aux)
    {
        ...
        aux.GetComponent<M9A79_drag>()._defaultPos = aux.GetComponent<RectTransform>().anchoredPosition;
        aux.GetComponent<M9A79_drag>().inDrop = false;
        aux.GetComponent<M9A79_drag>().swap = false;
        aux.GetComponent<M9A79_drag>()._drop = null;
        aux.GetComponent<M9A79_drag>().ItsInfinite = true;
