...
public class M9A79_toggle : MonoBehaviour
{
    ControlAudio _controlAudio;
    Toggle _toggle;
    [Header("is Right?")] public bool isRight;
    M9A79_grupo _behaviourToggleGroup;
    M9A79_managerSelccionar _managerToggle;

    void Start()
    {
        _controlAudio = FindObjectOfType<ControlAudio>();
        _toggle = GetComponent<Toggle>();
        _behaviourToggleGroup = transform.parent.GetComponent<M9A79_grupo>();
        _managerToggle = transform.parent.parent.GetComponent<M9A79_managerSelccionar>();
        _toggle.onValueChanged.AddListener(delegate { SetStateToggle(); });

        if (_managerToggle._NeedSymbol == M9A79_managerSelccionar.NeedSymbol.withSymbol)
            ...
            case M9A79_managerSelccionar.TypeSelection.onexgroup:
                ...
                if (_toggle.isOn)
                {
                    ...
                    switch (_managerToggle._typeVal)
                    {
                        ...
                        case M9A79_managerSelccionar.TypeValidation.inmediata:
