...
public class M8A51_managerInput : MonoBehaviour
{
    public Vector3 inicial, balini;
    ...
    public List<M8A51_group> _groupInputField;

    [HideInInspector] public List<InputField> ListaInputFields;
    ...
    public string animationNameRight, animationNameWrong, animationIdle;
    ...
    void Start()
    {
        ...
        inicial = cue.GetComponent<RectTransform>().anchoredPosition;
    }
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M8A51_input>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M8A51_input>()._isRight = false;
                inputField.GetComponent<M8A51_input>()._isEnabled = true;
    ...
    {
       //_controlNavegacion.Forward(3);
        //cue.transform.parent.GetComponent<Animator>().enabled = false;
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == M8A51_group.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<M8A51_input>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<M8A51_input>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<M8A51_input>()._isRight;
        ...
        if (groupTemp == _groupInputField.Count) {
            ...
            cue.transform.parent.GetComponent<Animator>().Play(animationNameRight+animationNumber);

        }
        else {
            _controlAudio.PlayAudio(2);
            cue.transform.parent.GetComponent<Animator>().Play(animationNameWrong + animationNumber);
        }
    ...
    {
        InitializeState();
        cue.transform.parent.GetComponent<Animator>().Play(animationIdle);

        //cue.transform.parent.GetComponent<Animator>().SetInteger("y",4);
        //cue.transform.parent.GetComponent<Animator>().SetInteger("y", 0);
