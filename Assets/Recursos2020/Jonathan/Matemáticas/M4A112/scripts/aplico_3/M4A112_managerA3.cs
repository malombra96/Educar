...
public class M4A112_managerA3 : MonoBehaviour
{
    public M4A112_A3 manager;
    ...
    public List<M4A112_groupA3> _groupInputField;
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M4A112_input>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    public void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M4A112_input>()._isRight = false;
                inputField.GetComponent<M4A112_input>()._isEnabled = true;
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == M4A112_groupA3.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<M4A112_input>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<M4A112_input>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<M4A112_input>()._isRight;
        ...
        if (groupTemp == _groupInputField.Count)
        {
            ...
            StartCoroutine( manager.Next(true));
            manager.correctas++;
            
        }
        else {
            _controlAudio.PlayAudio(2);
            StartCoroutine(manager.Next(false));
        }
