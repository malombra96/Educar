...
public class M07L122_ManagerInputX : MonoBehaviour
{
    public M07L122_Shape m07L122_Shape;
    ...
    [HideInInspector] public List<M07L122_GroupInputX> _groupInputField;
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M07L122_InputX>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    public void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M07L122_InputX>()._isRight = false;
                inputField.GetComponent<M07L122_InputX>()._isEnabled = true;
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == M07L122_GroupInputX.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<M07L122_InputX>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<M07L122_InputX>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
                    f.GetComponent<M07L122_InputX>()._isRight = answer;
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<M07L122_InputX>()._isRight;
       ...
       /*  if (_TipoPuntuacion == TipoPuntuacion.grupo)
        ...
        if((groupTemp == _groupInputField.Count) && m07L122_Shape._isRight)
        {
