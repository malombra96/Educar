...
public class M07L122_GroupInputX : MonoBehaviour
{
        ...
        for (int i = 0; i < _inputFields.Count; i++)
        {
            for (int j = 0; j < temporary.Count; j++)
            {
                if (temporary.Contains(_inputFields[i].text))
                {
                    ...
                    {
                        _inputFields[i].GetComponent<M07L122_InputX>()._isRight = true;
                ...
                {
                    _inputFields[i].GetComponent<M07L122_InputX>()._isRight = false;
                    break;
                }   
