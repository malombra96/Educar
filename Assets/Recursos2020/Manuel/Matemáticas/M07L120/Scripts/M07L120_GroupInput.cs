...
public class M07L120_GroupInput : MonoBehaviour
{
    M07L120_ManagerInput _managerInputField;
    ...
    void Awake()
    {
        _managerInputField = FindObjectOfType<M07L120_ManagerInput>();
        ...
        for (int i = 0; i < _inputFields.Count; i++)
        {
            for (int j = 0; j < temporary.Count; j++)
            {
                if (temporary.Contains(_inputFields[i].text))
                {
                    ...
                    {
                        _inputFields[i].GetComponent<M07L120_Input>()._isRight = true;
                ...
                {
                    _inputFields[i].GetComponent<M07L120_Input>()._isRight = false;
                    break;
                }   
