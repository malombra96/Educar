...
public class M06L090_GroupInput : MonoBehaviour
{
    M06L090_ManagerDropDown _managerInputField;
    ...
    void Awake()
    {
        _managerInputField = FindObjectOfType<M06L090_ManagerDropDown>();
        ...
        for (int i = 0; i < _inputFields.Count; i++)
        {
            for (int j = 0; j < temporary.Count; j++)
            {
                if (temporary.Contains(_inputFields[i].text))
                {
                    ...
                    {
                        _inputFields[i].GetComponent<M06L090_BehaviourInputField>()._isRight = true;
                ...
                {
                    _inputFields[i].GetComponent<M06L090_BehaviourInputField>()._isRight = false;
                    break;
                }   
