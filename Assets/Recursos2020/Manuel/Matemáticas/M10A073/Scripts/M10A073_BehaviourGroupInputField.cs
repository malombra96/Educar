...
public class M10A073_BehaviourGroupInputField : MonoBehaviour
{
    M10A073_ManagerInputField _managerInputField;
    ...
    void Awake()
    {
        _managerInputField = FindObjectOfType<M10A073_ManagerInputField>();
        ...
        for (int i = 0; i < _inputFields.Count; i++)
        {
            for (int j = 0; j < temporary.Count; j++)
            {
                if (temporary.Contains(_inputFields[i].text))
                {
                    ...
                    {
                        _inputFields[i].GetComponent<M10A073_BehaviourInputField>()._isRight = true;
                ...
                {
                    _inputFields[i].GetComponent<M10A073_BehaviourInputField>()._isRight = false;
                    break;
                }   
