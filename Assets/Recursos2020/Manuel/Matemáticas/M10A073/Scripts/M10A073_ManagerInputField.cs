...
public class M10A073_ManagerInputField : MonoBehaviour
{
    ...
    [HideInInspector] public List<M10A073_BehaviourGroupInputField> _groupInputField;
    ...
    [Header("M10A073 Setup & Parametres")]

    public GameObject _result;
    public GameObject[] _opportunities;
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M10A073_BehaviourInputField>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    public void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M10A073_BehaviourInputField>()._isRight = false;
                inputField.GetComponent<M10A073_BehaviourInputField>()._isEnabled = true;
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == M10A073_BehaviourGroupInputField.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = (f.GetComponent<M10A073_BehaviourInputField>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
                    ...
                    if(answer)
                    {
                        ...
                        f.GetComponent<M10A073_BehaviourInputField>()._isEnabled = false;
                    }
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<M10A073_BehaviourInputField>()._isRight;
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            {
                _opportunities[lifes].GetComponent<Image>().sprite = _opportunities[lifes].GetComponent<BehaviourSprite>()._wrong;
                lifes--;
    ...
    public void resetAll()
    {
        ...
        for (int i = 0; i < _opportunities.Length; i++)
            _opportunities[i].GetComponent<Image>().sprite = _opportunities[i].GetComponent<BehaviourSprite>()._default;
    }
