...
public class M10A073_ManagerDragDrop : MonoBehaviour
{
    ...
    [Header("Item List")]
    /* [HideInInspector] */ public List<M10A073_BehaviourDrag> _drags;
    [HideInInspector] public List<M10A073_BehaviourDrop> _drops;
    ...
    IEnumerator RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<M10A073_BehaviourDrag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
        {
            drag.GetComponent<M10A073_BehaviourDrag>().enabled = false;

            if(drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<M10A073_BehaviourDrop>())));
        }
        ...
        foreach (var drop in _drops)
            if(drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<M10A073_BehaviourDrag>()._DropRight.Contains(drop)));

        foreach (var answer in answers)
        {
            var BhDrop = answer.Key.GetComponent<M10A073_BehaviourDrop>();
    ...
    public void ImmediatelyValidation(M10A073_BehaviourDrag drag,M10A073_BehaviourDrop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M10A073_BehaviourDrag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M10A073_BehaviourDrop>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    public void SetPuntaje()
    {
        ...
        foreach (var answer in answers)
        {
            if (answer.Key.GetComponent<M10A073_BehaviourDrag>() && answer.Value)
                rights++;
        }
        ...
        else
        {
            ...
            _graphic.SetActive(rights == _customValidar);
    ...
    public void ResetDragDrop()
    {
        ...
        foreach (var drag in _drags)
        {
            ...
            if((_OperatingMethod == M10A073_ManagerDragDrop.OperatingMethod.Create) && !drag.GetComponent<M10A073_BehaviourDrag>().ItsInfinite)
                DestroyImmediate(drag.gameObject); 
        }

        if(_OperatingMethod == M10A073_ManagerDragDrop.OperatingMethod.Create)
