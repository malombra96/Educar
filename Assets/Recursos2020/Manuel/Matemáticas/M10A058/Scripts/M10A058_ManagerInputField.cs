...
public class M10A058_ManagerInputField : MonoBehaviour
{
    ...
    [HideInInspector] public List<M10A058_BehaviourGroupInputField> _groupInputField;
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M10A058_BehaviourInputField>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    public void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M10A058_BehaviourInputField>()._isRight = false;
                inputField.GetComponent<M10A058_BehaviourInputField>()._isEnabled = true;
                inputField.GetComponent<Image>().color = inputField.GetComponent<M10A058_BehaviourInputField>().colorFondoInicial;
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == M10A058_BehaviourGroupInputField.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<M10A058_BehaviourInputField>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<M10A058_BehaviourInputField>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<M10A058_BehaviourInputField>()._isRight;
