...
public class M04A113_GroupInput : MonoBehaviour
{
    M04A113_ManagerInput _managerInputField;
    ...
    void Awake()
    {
        _managerInputField = FindObjectOfType<M04A113_ManagerInput>();
        ...
        for (int i = 0; i < _inputFields.Count; i++)
        {
            for (int j = 0; j < temporary.Count; j++)
            {
                if (temporary.Contains(_inputFields[i].text))
                {
                    ...
                    {
                        _inputFields[i].GetComponent<M04A113_Input>()._isRight = true;
                ...
                {
                    _inputFields[i].GetComponent<M04A113_Input>()._isRight = false;
                    break;
                }   
