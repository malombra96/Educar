...
public class M04A113_ManagerInput : MonoBehaviour
{
    ...
    [HideInInspector] public List<M04A113_GroupInput> _groupInputField;
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M04A113_Input>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    public void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M04A113_Input>()._isRight = false;
                inputField.GetComponent<M04A113_Input>()._isEnabled = true;
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == M04A113_GroupInput.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<M04A113_Input>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<M04A113_Input>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
                    f.GetComponent<M04A113_Input>()._isRight = answer;
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<M04A113_Input>()._isRight;
