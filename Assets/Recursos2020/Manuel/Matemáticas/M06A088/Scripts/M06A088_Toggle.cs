...
public class M06A088_Toggle : MonoBehaviour
{
    ControlAudio _controlAudio;
    Toggle _toggle;
    [Header("is Right?")] public bool isRight;
    M06A088_BehaviourTG _behaviourToggleGroup;
    M06A088_ManagerST _managerToggle;

    void Start()
    {
        _controlAudio = FindObjectOfType<ControlAudio>();
        _toggle = GetComponent<Toggle>();
        _behaviourToggleGroup = transform.parent.GetComponent<M06A088_BehaviourTG>();
        _managerToggle = transform.parent.parent.GetComponent<M06A088_ManagerST>();
        _toggle.onValueChanged.AddListener(delegate{SetStateToggle();}); 

        if(_managerToggle._NeedSymbol == M06A088_ManagerST.NeedSymbol.withSymbol)
            ...
            case M06A088_ManagerST.TypeSelection.onexgroup:
                ...
                if (_toggle.isOn)
                {
                    ...
                    switch (_managerToggle._typeVal)
                    {
                        ...
                        case M06A088_ManagerST.TypeValidation.inmediata:
