...
public class M06A088_Drop : MonoBehaviour, IDropHandler
{
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        if (x != null && _drag == null)
        {
            ...
            if (_drag.GetComponent<M06A088_Drag>())
            {
                UpdateSlotDrop();
                ...
                _drag.GetComponent<M06A088_Drag>().inDrop = true;
                UpdateRowDrag(_drag);
                GetCalificationType();
            }
            else
    ...
    void UpdateSlotDrop()
    {
        if (_drag.GetComponent<M06A088_Drag>()._drop == null)
        {
            _drag.GetComponent<M06A088_Drag>()._drop = gameObject;
        }
        else
        {
            var previousDrop = _drag.GetComponent<M06A088_Drag>()._drop.GetComponent<M06A088_Drop>();
    ...
    void UpdateRowDrag(GameObject other)
    {
        if(_managerDragDrop._OperatingMethod == M06A088_ManagerDD.OperatingMethod.Match)
        {
            ...
            Vector2 def = other.GetComponent<M06A088_Drag>()._defaultPos;
            other.GetComponent<M06A088_Drag>()._row = this._row;

            foreach (var drag in _managerDragDrop._drags)
            {
                if (drag._row == other.GetComponent<M06A088_Drag>()._row && drag.gameObject != other)
                {
                    drag._row = x;
                    drag.GetComponent<RectTransform>().anchoredPosition = def;
                    drag.GetComponent<M06A088_Drag>().SwapDefaultPosition(other);
                    drag.UpdateCurrentPosition();
                    other.GetComponent<M06A088_Drag>().UpdateCurrentPosition();
                }
            }
        }
    ...
    {
        if (_managerDragDrop._TypeValidation == M06A088_ManagerDD.TypeValidation.Inmediata)
            _managerDragDrop.ImmediatelyValidation(_drag.GetComponent<M06A088_Drag>(),this);
        else
