...
public class M09A129_ManagerInput : MonoBehaviour
{
    [Header("World Manager")] M09A129_ControlWorld _ControlWorld;
    ...
    /* [HideInInspector] */ public List<M09A129_GroupInput> _groupInputField;
    ...
    {   
        ...
        _ControlWorld = FindObjectOfType<M09A129_ControlWorld>();
    } 
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M09A129_Input>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    public void InitializeState()
    {
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M09A129_Input>()._isRight = false;
                inputField.GetComponent<M09A129_Input>()._isEnabled = true;
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == M09A129_GroupInput.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<M09A129_Input>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<M09A129_Input>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
                    f.GetComponent<M09A129_Input>()._isRight = answer;
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<M09A129_Input>()._isRight;
        ...
        else
        {
            ...
            if(_ControlWorld._lifes < 0)
            {
                _controlNavegacion.GoToLayout(12,2);
            }
            else
            {
                _ControlWorld._BehaviourPlayer.SetBehaviourPlayer(true);
