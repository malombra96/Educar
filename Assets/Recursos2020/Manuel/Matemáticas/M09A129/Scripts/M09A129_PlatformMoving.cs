...
public class M09A129_PlatformMoving : MonoBehaviour
{
    [Header("WayPoints")] 
    ...
    void Update()
    {
        ...
        if(Vector2.Distance(_rectTransform.anchoredPosition,_B.anchoredPosition) < 1)
        {
            if(_C)
        ...
        if(_C)
        {
            if(Vector2.Distance(_rectTransform.anchoredPosition,_C.anchoredPosition) < 1)
            _next = _A.anchoredPosition;
        }
