...
public class M09A060_BehaviourToggle : MonoBehaviour
{
    ControlAudio _controlAudio;
    Toggle _toggle;
    [Header("is Right?")] public bool isRight;
    M09A060_BehaviourToggleGroup _behaviourToggleGroup;
    M09A060_ManagerSeleccionarToggle _managerToggle;

    void Start()
    {
        _controlAudio = FindObjectOfType<ControlAudio>();
        _toggle = GetComponent<Toggle>();
        _behaviourToggleGroup = transform.parent.GetComponent<M09A060_BehaviourToggleGroup>();
        _managerToggle = transform.parent.parent.GetComponent<M09A060_ManagerSeleccionarToggle>();
        _toggle.onValueChanged.AddListener(delegate{SetStateToggle();}); 

        if(_managerToggle._NeedSymbol == M09A060_ManagerSeleccionarToggle.NeedSymbol.withSymbol)
        {
            ...
            case M09A060_ManagerSeleccionarToggle.TypeSelection.onexgroup:
                ...
                if (_toggle.isOn)
                {
                    ...
                    switch (_managerToggle._typeVal)
                    {
                        ...
                        case M09A060_ManagerSeleccionarToggle.TypeValidation.inmediata:
