...
public class M09A060_ManagerIF : MonoBehaviour
{
    ...
    [HideInInspector] public List<M09A060_BGroupIF> _groupInputField;
    ...
    [Header("Manager Drag&Drop")] public M09A060_ManagerDD _ManagerDD;

    [Header("State Answer")] public bool _stateAnswer;
    ...
    {   
        ...
        _ManagerDD = GetComponent<M09A060_ManagerDD>();
    } 
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M09A060_BInputField>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    public void InitializeState()
    {
        ...
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M09A060_BInputField>()._isRight = false;
                inputField.GetComponent<M09A060_BInputField>()._isEnabled = true;
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == M09A060_BGroupIF.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<M09A060_BInputField>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<M09A060_BInputField>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<M09A060_BInputField>()._isRight;
        ...
        _stateAnswer = (groupTemp == _groupInputField.Count);
        _ManagerDD.ValidarDragDrop();
