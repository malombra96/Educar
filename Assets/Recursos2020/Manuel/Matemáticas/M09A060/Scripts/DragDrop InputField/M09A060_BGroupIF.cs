...
public class M09A060_BGroupIF : MonoBehaviour
{
    M09A060_ManagerIF _managerInputField;
    ...
    void Awake()
    {
        _managerInputField = FindObjectOfType<M09A060_ManagerIF>();
        ...
        for (int i = 0; i < _inputFields.Count; i++)
        {
            for (int j = 0; j < temporary.Count; j++)
            {
                if (temporary.Contains(_inputFields[i].text))
                {
                    ...
                    {
                        _inputFields[i].GetComponent<M09A060_BInputField>()._isRight = true;
                ...
                {
                    _inputFields[i].GetComponent<M09A060_BInputField>()._isRight = false;
                    break;
                }   
