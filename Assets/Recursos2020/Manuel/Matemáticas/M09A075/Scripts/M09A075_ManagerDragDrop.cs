...
public class M09A075_ManagerDragDrop : MonoBehaviour
{
    ...
    [Header("Item List")]
    [HideInInspector] public List<M09A075_BehaviourDrag> _drags;
    [HideInInspector] public List<M09A075_BehaviourDrop> _drops;

    [HideInInspector] public List<M09A075_BehaviourDropGroup> _groups;
    ...
    IEnumerator RandomDrag()
    {
        ...
        for (int j = 0; j < _drags.Count; j++)
        {
            ...
            if (statePos != posnew)
            {
                ...
                _drags[posnew] = randomTemp.GetComponent<M09A075_BehaviourDrag>();
            }
        }
    ...
    void ValidarDragDrop()
    {
        ...
        foreach (var drag in _drags)
            if(drag._drop)
                answers.Add(drag.gameObject, (drag._DropRight.Contains(drag._drop.GetComponent<M09A075_BehaviourDrop>())));


        foreach (var drop in _drops)
            if(drop._drag)
                answers.Add(drop.gameObject, (drop._drag.GetComponent<M09A075_BehaviourDrag>()._DropRight.Contains(drop)));

        foreach (var answer in answers)
        {
            var BhDrop = answer.Key.GetComponent<M09A075_BehaviourDrop>();
            ...
            if (BhDrop)
            {
                ...
                {
                    if (answer.Value)
                        BhDrop._group.GetComponent<M09A075_BehaviourDropGroup>()._currentRight++;
                    else
                        BhDrop._group.GetComponent<M09A075_BehaviourDropGroup>()._currentWrong++;
                }
            }
            //print(answer.Key.GetComponent<M09A075_BehaviourDrag>()._group+""+answer.Key + answer);
        }
    ...
    public void ImmediatelyValidation(M09A075_BehaviourDrag drag,M09A075_BehaviourDrop drop)
    {
    ...
    {
        foreach (var answer in answers)
            if (answer.Key.GetComponent<M09A075_BehaviourDrag>())
                SetSpriteAnswer(answer.Key.GetComponent<Image>(), answer.Value);
    }
    ...
    public void SetPuntaje()
    {
        ...
        foreach (var answer in answers)
        {
            M09A075_BehaviourDrag temp = answer.Key.GetComponent<M09A075_BehaviourDrag>();
        ...
        else
        {
            if(_customValidar == 0)
                ...
                _controlAudio.PlayAudio((rights == _customValidar) && (wrongs == 0)? 1 : 2);

            int p = rights - wrongs;

            _controlPuntaje.IncreaseScore(p>0? p : 0);
            _controlNavegacion.Forward(2);
        }   
