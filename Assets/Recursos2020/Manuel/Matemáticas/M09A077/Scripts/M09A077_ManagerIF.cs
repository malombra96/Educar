...
public class M09A077_ManagerIF : MonoBehaviour
{
    ...
    [HideInInspector] public List<M09A077_BehaviourGI> _groupInputField;
    ...
    [Header("Graphic Image")] public GameObject _graphic;
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M09A077_BehaviourIF>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    public void InitializeState()
    {
        ...
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M09A077_BehaviourIF>()._isRight = false;
                inputField.GetComponent<M09A077_BehaviourIF>()._isEnabled = true;
                inputField.GetComponent<Image>().color = colorFondoInicial;

                inputField.GetComponent<M09A077_BehaviourIF>()._point.SetActive(false);
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == M09A077_BehaviourGI.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<M09A077_BehaviourIF>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<M09A077_BehaviourIF>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
                    f.GetComponent<M09A077_BehaviourIF>()._isRight = answer;
                    _listAnswers[i].inputFields[j] = f;
                    _listAnswers[i].answers[j] = answer;

                    f.GetComponent<M09A077_BehaviourIF>()._point.SetActive(answer);
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<M09A077_BehaviourIF>()._isRight;
