...
public class M09A077_BehaviourGI : MonoBehaviour
{
    M09A077_ManagerIF _managerInputField;
    ...
    void Awake()
    {
        _managerInputField = FindObjectOfType<M09A077_ManagerIF>();
        ...
        for (int i = 0; i < _inputFields.Count; i++)
        {
            for (int j = 0; j < temporary.Count; j++)
            {
                if (temporary.Contains(_inputFields[i].text))
                {
                    ...
                    {
                        _inputFields[i].GetComponent<M09A077_BehaviourIF>()._isRight = true;
                ...
                {
                    _inputFields[i].GetComponent<M09A077_BehaviourIF>()._isRight = false;
                    break;
                }   
