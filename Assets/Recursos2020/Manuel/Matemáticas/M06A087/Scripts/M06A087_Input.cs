...
public class M06A087_Input : MonoBehaviour,IPointerClickHandler
{
    M06A087_ManagerInput _managerInputField;
    InputField _input;
    ...
    [Header("Text 2")] public Text _sizeV;

    [Header("Multipliquer Ramp")] public int _multipliquer;
    [Header("Vetical Size Ramp")] public Image _rampV;
    [Header("State Answer")] public string _stateAnswer;
    ...
    void Awake()
    {
        ...
        _managerInputField = FindObjectOfType<M06A087_ManagerInput>();
    ...
    public void SetEmptyStandard(string s)
    {
        ...
        float rampH = _rampV.GetComponent<RectTransform>().sizeDelta.x;

        if(int.TryParse(_input.text, out int v))
        {
            _rampV.GetComponent<RectTransform>().sizeDelta = new Vector2( rampH, _multipliquer*v);
            int rr = int.Parse(respuestaCorrecta[0]);

            _stateAnswer = v == rr? "Equals" : v > rr? "Greater" : "Less";

            _sizeV.text = _input.text;
        }
        else
        {
            _rampV.GetComponent<RectTransform>().sizeDelta = new Vector2(rampH, 0);
            _stateAnswer = "";
        }
