...
public class M06A087_ManagerInput : MonoBehaviour
{
    ...
    [HideInInspector] public List<M06A087_GroupInput> _groupInputField;
    ...
    [Header("Biker")] public Animator _biker;
    ...
    public void SetStateValidarBTN()
    {
        ...
        foreach (var group in _groupInputField)
        {
            ...
            {
                if (inputField.GetComponent<M06A087_Input>()._isEmpty)
                {
                    s = false;
                    break;
                }
    ...
    public void InitializeState()
    {
        ...
        foreach (var group in _groupInputField)
        {
            foreach (var inputField in group._inputFields)
            {
                ...
                inputField.GetComponent<M06A087_Input>()._isRight = false;
                inputField.GetComponent<M06A087_Input>()._isEnabled = true;
                inputField.GetComponent<Image>().color = colorFondoInicial;

                float rampH = inputField.GetComponent<M06A087_Input>()._rampV.GetComponent<RectTransform>().sizeDelta.x;
                inputField.GetComponent<M06A087_Input>()._rampV.GetComponent<RectTransform>().sizeDelta = new Vector2(rampH, 0);
                inputField.GetComponent<M06A087_Input>()._stateAnswer = "";
    ...
    public void CalificarInputs()
    {
        ...
        for (int i = 0; i < _groupInputField.Count; i++)
        {
            ...
            if (_groupInputField[i]._TipoRespuestas == M06A087_GroupInput.TipoRespuestas.individual)
            {
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    f.GetComponent<M06A087_Input>()._isEnabled = false;

                    bool answer;
                    answer = (f.GetComponent<M06A087_Input>().respuestaCorrecta.Contains(f.GetComponent<InputField>().text));
                    f.GetComponent<M06A087_Input>()._isRight = answer;
                    _listAnswers[i].inputFields[j] = f;
                    _listAnswers[i].answers[j] = answer;

                    if(f.GetComponent<M06A087_Input>()._stateAnswer == "Less")
                        _biker.SetInteger("answer", 1);
                    else if (f.GetComponent<M06A087_Input>()._stateAnswer == "Equals")
                        _biker.SetInteger("answer", 2);
                    else
                        _biker.SetInteger("answer", 3);
            ...
            else
            {
                ...
                for (int j = 0; j < _groupInputField[i]._inputFields.Count; j++)
                {
                    ...
                    bool answer;
                    answer = f.GetComponent<M06A087_Input>()._isRight;
